	...
	if Unit:GetEntry() == 25588 then
		...
		for k,v in pairs(blueorbs) do
			if v == 1 then
				for i = 1,table.getn(spawnpts.x) do
					table.insert(blueorbs,Unit:SpawnGameObject(188116,spawnpts.x[i],spawnpts.y[i],spawnpts.z[i],spawnpts.o[i],0))
				end
				break
			end
		end
		for k,v in pairs(startbl) do
			if v == 1 then
				...
				table.insert(startbl,Unit:SpawnCreature(25608,1699.204712,628.331482,27.539782-1,Unit:GetO(),Unit:GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE),0))
...
function KilJaedenDummy_AIUpdate(Unit)
	if Unit:GetEntry() == 25608 then
		...
		if no ~= nil and no == 0 then
			cn_kiljaeden = Unit:SpawnCreature(kiljaeden,Unit:GetX(),Unit:GetY(),Unit:GetZ()+1.7,Unit:GetO(),Unit:GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE),0)
			Unit:RemoveEvents()
			Unit:RemoveAIUpdateEvent()
			Unit:RemoveFromWorld()
		end
...
function KilJaeden_OnSpawn(Unit)
	...
	Unit:SetCombatCapable(1)
	Unit:SetCombatMeleeCapable(1)
	Unit:CastSpell(37745) -- birth emote
...
function KilJaeden_OnCombat(Unit)
	...
	kalec = Unit:SpawnCreature(25319,cn_kiljaeden:GetX(),cn_kiljaeden:GetY(),cn_kiljaeden:GetZ()+100,Unit:GetO(),35,0)
end
...
function KilJaeden_Gossip(Unit)
	...
	elseif saytable.orbsay1 == true and kalec ~= nil and m_phase == 2 then
		...
		if chosenorb ~= nil then
			table.insert(activatedorbs,chosenorb);
	...
	elseif saytable.kalecsay4 == true and saytable.anveenasay4 == true and saytable.phase5say == true and anveena ~= nil then
		...
		anveena:FullCastSpellOnTarget(46474,Unit:GetGUID())
...
function KilJaeden_AIUpdate(Unit)
	...
	if hp > 85 then
		if soul_flay == 5 and cancast == true and Unit:GetMainTank() then
			cancast = false
			Unit:FullCastSpellOnTarget(45442,Unit:GetMainTank())
			soul_flay = 0
			Unit:RegisterEvent("KilJaeden_CanCast",3500, 1)
		elseif soul_flay == 5 and cancast == false and Unit:GetMainTank() then
			soul_flay = soul_flay-1
		end
		...
		if legion_light == 10 and cancast == true and Unit:GetRandomPlayer(0)then
			cancast = false
			Unit:FullCastSpellOnTarget(45664,Unit:GetRandomPlayer(0))
			legion_light = 0
			Unit:RegisterEvent("KilJaeden_CanCast",3000,1)
		elseif legion_light == 10 and cancast == false and Unit:GetRandomPlayer(0) then
			legion_light = legion_light-1
		end
	...
	elseif hp <= 85 and hp > 55 then
		...
		if sinister_reflect == true and cancast == true then
			...
			cancast = false
			Unit:RegisterEvent("KilJaeden_SinisterReflect",1500,1)
		end
		if shadow_spike == true and cancast == true and Unit:GetRandomPlayer(0) then
			cancast = false
			shadow_spike = false
		...
		if legion_light >= 15 and Unit:GetRandomPlayer(0)and cancast == true then
			cancast = false
			Unit:FullCastSpellOnTarget(45664,Unit:GetRandomPlayer(0))
			legion_light = 0
			Unit:RegisterEvent("KilJaeden_CanCast",3000, 1)
		...
		if flame_dart >= 30 and cancast == true and Unit:GetRandomPlayer(0) then
			cancast = false
			Unit:FullCastSpellOnTarget(45737,Unit:GetRandomPlayer(0))
			flame_dart = 0
			Unit:RegisterEvent("KilJaeden_CanCast",2000, 1)
		elseif  flame_dart >= 30 and cancast == false then
			flame_dart = flame_dart - 1
		end
		if soul_darkness >= 80 and cancast == true then
			cancast = false
			Unit:FullCastSpell(46605)
			Unit:SendChatMessage(CHAT_MSG_RAID_WARNING,LANG_UNIVERSAL,Unit:GetName().." begins to channel dark energy!")
			soul_darkness = 0
			Unit:RegisterEvent("KilJaeden_CanCast",10000, 1)
			Unit:RegisterEvent("KilJaeden_SoulDarkness",8800,1)
		elseif soul_darkness >= 80 and cancast == false then
			soul_darkness = soul_darkness-1
		end
	elseif hp <= 55 and hp > 25 then
		...
		if meteor_timer >= 3 and meteors <= 3 and castmeteor == true then
			local plr = Unit:GetRandomPlayer(0)
			if plr~= nil then
		...
		if shadow_spike == true and cancast == true then
			cancast = false
			Unit:FullCastSpell(46680)
			shadow_spike = false
	...
	elseif hp  <= 25 then
		...
		if meteor_timer >= 3 and meteors <= 5 and castmeteor == true then
			local plr = Unit:GetRandomPlayer(0)
			if plr~= nil then
				Unit:SpawnCreature(25735,plr:GetX(),plr:GetY(),plr:GetZ()+30,plr:GetO(),16,8000)
				Unit:SpawnCreature(60000,plr:GetX(),plr:GetY(),plr:GetZ(),plr:GetO(),16,8000)
		...
		if cancast == true and soul_flay >= 5 and Unit:GetMainTank() then
			cancast = false
			Unit:FullCastSpellOnTarget(45442,Unit:GetMainTank())
			soul_flay = 0
			Unit:RegisterEvent("KilJaeden_CanCast",3500, 1)
...
function KilJaeden_SinisterReflect(Unit)
	for i = 1,4 do
		local plr = Unit:GetRandomPlayer(0)
		if plr ~= nil then
			Unit:SpawnCreature(25708,plr:GetX(),plr:GetY(),plr:GetZ(),plr:GetO(),16,0)
		end
	end
	Unit:Emote(0,0)
	Unit:Emote(404,0)
...
function KilJaeden_InitiliazeMeteor(Unit)
	Unit:MoveTo(Unit:GetX(),Unit:GetY(),Unit:GetZ(),Unit:GetO())
	Unit:EnableFlight()
	Unit:SetUInt32Value(UNIT_FIELD_FLAGS,UNIT_FLAG_NOT_ATTACKABLE_9)
	Unit:SetUInt32Value(UNIT_FIELD_FLAGS,UNIT_FLAG_NOT_SELECTABLE)
	Unit:DisableRespawn()
...
function InitializeSinisterReflection(Unit)
	...
	Unit:SetUInt32Value(UNIT_FIELD_DISPLAYID,tar:GetUInt32Value(UNIT_FIELD_NATIVEDISPLAYID))
	Unit:SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY, tar:GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY))
	Unit:SetUInt32Value(UNIT_VIRTUAL_ITEM_INFO,tar:GetUInt32Value(UNIT_VIRTUAL_ITEM_INFO))
	Unit:SetUInt32Value(UNIT_VIRTUAL_ITEM_INFO_01,tar:GetUInt32Value(UNIT_VIRTUAL_ITEM_INFO_01))
	Unit:SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY_01,tar:GetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_DISPLAY_01))
	Unit:SetUInt32Value(UNIT_VIRTUAL_ITEM_INFO_02,tar:GetUInt32Value(UNIT_VIRTUAL_ITEM_INFO_02))
	Unit:SetUInt32Value(UNIT_VIRTUAL_ITEM_INFO_03,tar:GetUInt32Value(UNIT_VIRTUAL_ITEM_INFO_03))
	--[[Unit:FullCastSpellOnTarget(45205,tar)
	Unit:FullCastSpellOnTarget(41054,tar)
	Unit:FullCastSpell(45785)]]
	local class = tar:GetPlayerClass()
	if class == "Warlock" then
		Unit:AddAISpell(46190,4,15000,0,0,100,0)
		Unit:AddAISpell(47076,4,7000,0,0,100,1)
...
function InitializeOrbOfBlue(Unit)
	Unit:ModUInt32Value(OBJECT_FIELD_SCALE_X,3)
	Unit:SetFloatValue(GAMEOBJECT_FACING,Unit:CalcRadAngle(Unit:GetX(),Unit:GetY(),1699.660767,630.153320))
	for k,v in pairs(blueorbs) do
		if v == 1 then
