...
bool ScrapReaver(uint32 i, Spell* pSpell)
{
	...
	Player *pPlayer = static_cast< Player* >( pSpell->p_caster );
...
bool BurnItUp(uint32 i, Spell* pSpell)
{
	...
	GameObject *west = pPlayer->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(-350, 2708, 35, 183122);
...
bool ThirstUnending(uint32 i, Spell* pSpell)
{
  ...
  QuestLogEntry *qle = pPlayer->GetQuestLogForEntry(8346);
  if(!qle)
	  return true;
