...
I will mostly deal with them later :)
*/
...
class Priestess_DelrissaAI : public MoonScriptBossAI
{
	...
	Priestess_DelrissaAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		...
		AddEmote(Event_OnCombatStart, "Annihilate them!", Text_Yell, 12395);
...
class Kagani_NightstrikeAI : public MoonScriptBossAI
{
	...
	Kagani_NightstrikeAI(Creature* pCreature) : MoonScriptBossAI(pCreature)
	{
		...
		AddSpell(KAGANI_NIGHTSTRIKE_Eviscerate, Target_Current, 80, 0, 7, 0, 30);
...
class SunbladeImpAI : public MoonScriptBossAI
{
	...
	{
		AddSpell(IMP_FIREBOLT, Target_Current, 100, 2, 2, 0, 30);
	}
};
...
//Sunblade Sentinel
/*
//Event Kalecgos Gossip for that orb...
#define GOSSIP_KALECGOS 24848

class KalecgosGS : public GossipScript
{
public:
	void Destroy()
	{
	...
	void GossipHello(Object* pObject, Player* plr, bool AutoSend)
	{
		...
		objmgr.CreateGossipMenuForPlayer(&Menu, pObject->GetGUID(), 8000005, plr);
		
		Menu->AddItem(0, "Who are you?", 1);
		Menu->SendTo(plr);
	}
	...
	void GossipSelectOption(Object* pObject, Player * plr, uint32 Id, uint32 IntId, const char * Code)
	{
		...
		{
		case 1:
		objmgr.CreateGossipMenuForPlayer(&Menu, pCreature->GetGUID(), 8000000, plr);
		Menu->AddItem(0, "What can we do to assist you", 2);
		Menu->SendTo(plr);
		break;

		case 2:
		objmgr.CreateGossipMenuForPlayer(&Menu, pCreature->GetGUID(), 8000001, plr);
		Menu->AddItem(0, "What brings you to the Sunwell?", 3);
		Menu->SendTo(plr);
		break;

		case 3:
		objmgr.CreateGossipMenuForPlayer(&Menu, pCreature->GetGUID(), 8000002, plr);
		Menu->AddItem(0, "You're not alone here?", 4);
		Menu->SendTo(plr);
		break;

		case 4:
		objmgr.CreateGossipMenuForPlayer(&Menu, pCreature->GetGUID(), 8000003, plr);
		Menu->AddItem(0, "What would Kil'jaeden want with a mortal woman?", 5);
		Menu->SendTo(plr);
		break;

		case 5:
		objmgr.CreateGossipMenuForPlayer(&Menu, pCreature->GetGUID(), 8000005, plr);
		Menu->SendTo(plr);
		break;
		}
	}
...
void SetupMagistersTerrace(ScriptMgr* pScriptMgr) 
{
	...
	//Gossips
	/*GossipScript * KalecgosGS = (GossipScript*) new KalecgosGS;
	mgr->register_gossip_script(GOSSIP_KALECGOS, KalecgosGS);*/
}
