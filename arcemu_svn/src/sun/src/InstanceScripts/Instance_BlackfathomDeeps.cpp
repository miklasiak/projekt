...
class LadySarevessAI : public MoonScriptCreatureAI
{
    ...
    LadySarevessAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature) 
	{
		...
		AddEmote(Event_OnCombatStart, "You should not be here! Slay them!", Text_Yell);
	}
};
...
public:
	...
	void OnActivate(Player * pPlayer)
	{
		...
		{
			// Spawn Baron Aquanis
			_gameobject->GetMapMgr()->GetInterface()->SpawnCreature(CN_BaronAquanis, -782.021f, -63.5876f, -45.0935f, -2.44346f, false, false, 0, 0);
			BaronAquanis = true;
