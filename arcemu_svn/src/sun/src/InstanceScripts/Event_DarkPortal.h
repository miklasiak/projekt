...
typedef std::vector< MoonScriptCreatureAI* >	AIContainer;
typedef std::set< MoonScriptCreatureAI* >		AISet;
...
public:
    ...
    DarkPortalEventTriggerAI( Creature *pCreature );// : MoonScriptCreatureAI( pCreature );
	...
	bool IsRegimentFull(AIContainer VectorSet, size_t SetSize);
	...
	bool					mHasLoadedCells;
	bool					mResurrected;
	bool					mResurrect;

	// Chain-aggro related variables
	UnitChain*				mFormationChain;

	// Creature AI pointers
	MoonScriptCreatureAI*	mPitCommander;
	MoonScriptCreatureAI*	mAlliance;
	MoonScriptCreatureAI*	mHorde;
	MoonScriptCreatureAI*	mJusti;
	MoonScriptCreatureAI*	mMelgr;

	AISet					mFightingDemonSet;
	AISet					mQueuedDemonSet;
	AISet					mInfernalTable;
...
{
	/* Event Trigger */
	mgr->register_creature_script(CN_DARK_PORTAL_EVENT_TRIGGER, &DarkPortalEventTriggerAI::Create);

	/* Horde Units */
	mgr->register_creature_script(CN_MELGROMM_HIGHMOUNTAIN, &MelGrommHighmountainAI::Create);
	mgr->register_creature_script(CN_ORGRIMMAR_GRUNT, &OrgrimmarGruntAI::Create);
	mgr->register_creature_script(CN_ORGRIMMAR_SHAMAN, &OrgrimmarShamanAI::Create);
	mgr->register_creature_script(CN_DARKSPEAR_AXE_THROWER, &DarkspearAxeThrowerAI::Create);
	mgr->register_creature_script(CN_UNDERCITY_MAGE, &UndercityMageAI::Create);

	/* Alliance Units */
	mgr->register_creature_script(CN_JUSTINIUS_THE_HARBINGER, &JustiniusTheHarbingerAI::Create);
	mgr->register_creature_script(CN_STORMWIND_SOLDIER, &StormwindSoldierAI::Create);
	mgr->register_creature_script(CN_IRONFORGE_PALADIN, &IronforgePaladinAI::Create);
	mgr->register_creature_script(CN_DARNASSIAN_ARCHER, &DarnassianArcherAI::Create);
	mgr->register_creature_script(CN_STORMWIND_MAGE, &StormwindMageAI::Create);

	/* Demon Units */
	mgr->register_creature_script(CN_PIT_COMMANDER, &PitCommanderAI::Create);
	mgr->register_creature_script(CN_INFERNAL_SIEGEBREAKER, &InfernalSiegebreakerAI::Create);
	mgr->register_creature_script(CN_FEL_SOLDIER, &FelSoldierAI::Create);
	mgr->register_creature_script(CN_WRATH_MASTER, &WrathMasterAI::Create);
}