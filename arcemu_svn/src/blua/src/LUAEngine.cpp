...
RegType<Unit> UnitMethods[] = {
	...
	{ "BroadcastMessage", &luaUnit_SendBroadcastMessage },
	...
	//custom commmands
	{ "IsPvPFlagged", &luaUnit_PvPFlagged },
	{ "GetModel", &luaUnit_GetModel },
	{ "GetPlayerSpeed", &luaUnit_PlrSpeed },
	{ "Disconnect", &luaUnit_Disconnect },
	{ "GetZone", &luaUnit_GetZone },
	{ "GetMap", &luaUnit_GetMap },
	{ "BroadcastWideMessage", &luaUnit_SendWideBroadcastMessage },
	{ "IsBehind", &luaUnit_IsBehind },
	{ "RemoveFlag", &luaUnit_RemoveFlag },
	{ "IsFlying", &luaUnit_IsFlying },
	{ "SetFactionStanding", &luaUnit_SetFactionStanding },
	{ "GetLevel", &luaUnit_GetLevel },
	{ "IsAlive", &luaUnit_IsAlive },
	{ "IsStealthed", &luaUnit_IsStealthed },
	{ "IsGM", &luaUnit_IsGM },
	{ "IsAlliance", &luaUnit_IsAlliance },
	{ "IsHorde", &luaUnit_IsHorde },
	{ "HasAmountOfPlayers", &luaUnit_HasAmountOfPlayers },
	{ "HasGMTagOn", &luaUnit_HasGMTagOn },
	{ "CastSpellAoF", &luaUnit_CastSpellAoF },
	{ "RemoveAllAuras", &luaUnit_RemoveAllAuras },
	{ "TogglePvP", &luaUnit_TogglePvP },
	{ "ApplyRestState", &luaUnit_ApplyRestState },
	{ "ResurrectPlayer", &luaUnit_ResurrectPlayer },
	{ "SetEmoteState", &luaUnit_SetEmoteState },
	{ "GetPlayerAccountName", &luaUnit_GetPlayerAccountName },
	{ "RemoveStealth", &luaUnit_RemoveStealth },
	{ "SetPVPRank", &luaUnit_SetPVPRank },
	{ "Die", &luaUnit_Die },
	{ "SetStealth", &luaUnit_SetStealth },
	{ "WorldQuery", &luaUnit_WorldQuery },
	{ "CharacterQuery", &luaUnit_CharacterQuery },
	{ "AddCreatureLoot", &luaUnit_AddCreatureLoot },
	{ "SystemMessageToPlayer", &luaUnit_SystemMessageToPlr },
	{ "IsClass", &luaUnit_IsClass },
	{ "SaveToDB", &luaUnit_SaveToDB },
	{ "IsInvincible", &luaUnit_IsInvincible },
	{ "IsAttacking", &luaUnit_IsAttacking },
	{ "SetInvincible", &luaUnit_SetInvincible },
	{ "IsInRange", &luaUnit_IsInRange },
	{ "FinishQuest", &luaUnit_FinishQuest },
	{ "SubtractTime", &luaUnit_SubtractTime },
	{ "IfQuestCanBeFinished", &luaUnit_IfQuestCanBeFinished },
	{ "SetQuestComplete", &luaUnit_SetQuestComplete },
	{ "MoveToPlayer", &luaUnit_MoveToPlayer },
	{ "EventCreatureYell", &luaUnit_EventCreatureYell },
	{ "EventCreatureDelete", &luaUnit_EventCreatureDelete },
	{ "EventCastSpell", &luaUnit_EventCastSpell },
	{ "EventCastSpellOnSelf", &luaUnit_EventCastSpellOnSelf },
	{ "EventPlaySound", &luaUnit_EventPlaySound },
	{ NULL, NULL },
};
...
void LuaEngine::OnUnitEvent(Unit * pUnit, const char * FunctionName, uint32 EventType, Unit * pMiscUnit, uint32 Misc)
{
	...
	if(lua_isnil(L,-1))
	{
		...
		sLog.outColor(TRED, "Tried to call invalid LUA function '%s' from ArcEmu (Unit)!\n", FunctionName);
		m_Lock.Release();
		return;
	}
...
void LuaEngine::OnQuestEvent(Player * QuestOwner, const char * FunctionName, uint32 QuestID, uint32 EventType, Object * QuestStarter)
{
	...
	{	
		...
		sLog.outColor(TRED, "Tried to call invalid LUA function '%s' from ArcEmu (Quest)!\n", FunctionName);
		m_Lock.Release();
		return;
	}
...
void LuaEngine::OnGameObjectEvent(GameObject * pGameObject, const char * FunctionName, uint32 EventType, Unit * pMiscUnit)
{
	...
	if(lua_isnil(L,-1))
	{
		...
		sLog.outColor(TRED, "Tried to call invalid LUA function '%s' from ArcEmu (GO)!\n", FunctionName);
		m_Lock.Release();
		return;
	}
...
void LuaEngine::OnGossipEvent(Object * pObject, const char * FunctionName, uint32 EventType, Player * mPlayer, uint32 Id, uint32 IntId, const char * Code)
{
	...
	if(lua_isnil(L, -1))
	{
		...
		sLog.outColor(TRED, "Tried to call invalid LUA function '%s' from ArcEmu (Gossip)!\n", FunctionName);
		m_Lock.Release();
		return;
	}
...
void LuaEngineMgr::RegisterServerHookEvent(uint32 Id, uint32 Event, const char *FunctionName)
{
	ServerHookBindingMap::iterator itr = m_sh_hookBinding.find(Id);

    if(itr == m_sh_hookBinding.end())
	{
		LuaServerHookBinding gb;
		memset(&gb, 0, sizeof(LuaServerHookBinding));
		gb.Functions[Event] = strdup(FunctionName);
		m_sh_hookBinding.insert(make_pair(Id, gb));
	}
	else
	{
		if(itr->second.Functions[Event]!=NULL)
...
{
	if(ptr==NULL) return 0;
	lua_pushnumber(L, (double)ptr->GetZoneId());
	return 1;
}
...
{
	if(ptr==NULL) return 0;
	lua_pushnumber(L, (double)ptr->GetMapMgr()->GetMapId());
	return 1;
}
...
int luaUnit_IsFlying(lua_State * L, Unit * ptr)
{
	Unit * target = Lunar<Unit>::check(L, 1);
	if(ptr && target)
		if(target->GetAIInterface()->IsFlying())
			lua_pushboolean(L, 1);
		else
			lua_pushboolean(L, 0);
	return 1;
...
{
	Player * plr = (Player*)ptr;
	char * rank = luaL_checkstring(L,1);
	
	if(!rank) 
		return 0;
	else
		if(!plr)
			plr->GetSession()->CanUseCommand(rank);
...
int luaUnit_HasAmountOfPlayers(lua_State * L, Unit * ptr)
{
	...
	if (sWorld.AlliancePlayers + sWorld.HordePlayers == Amount)
	{
		lua_pushboolean(L, 1);
		return 1;
	}
...
int luaUnit_ApplyRestState(lua_State * L, Unit * ptr)
{
	Unit * Target = Lunar<Unit>::check(L, 1);
	Player * plr = (Player*)Target;
	bool rs = (bool)luaL_checkint(L, 1);
	if(ptr && plr)
	{
		if(rs== 0)
			plr->ApplyPlayerRestState(false);
		else if(rs== 1)
...
{
	int Class = luaL_checkint(L, 1);
	Player * Plr = (Player*)ptr;
	if(Plr)
	{
		if(Plr->getClass() == Class)
		{
			lua_pushboolean(L, 1);
		return 1;
		}
...
int luaUnit_FinishQuest(lua_State * L, Unit * ptr)
{
	...
	qle = plr->GetQuestLogForEntry(questid);
	if(qle)
	qle->Finish();
...
//Test Box Here
int luaUnit_TestFunction(lua_State * L, Unit * ptr)
{
	Player * plr = (Player*)ptr;
	Creature * cre = (Creature*)ptr;
	QuestLogEntry *qle;
