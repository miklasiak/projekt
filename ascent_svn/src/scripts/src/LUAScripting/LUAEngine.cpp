...
template<>
const char * GetTClassName<Object>()
{
	return "Object";
}
...
RegType<Unit> UnitMethods[] = {
	...
	{ "DisableFlight", &luaUnit_Land },
	{ "EnableFlight", &luaUnit_SetFlying },
	...
	{ "MonsterMove", &luaUnit_MonsterMove },
	{ "SetPosition", &luaUnit_SetPosition },
	{ "AddAISpell", &luaUnit_AddAISpell },
	{ "PlayerCastSpell", &luaUnit_PlayerCastSpell },
	{ "PlayerCastSpellOnTarget", &luaUnit_PlayerCastSpellOnTarget },
	{ "RegisterAIUpdateEvent", &luaUnit_RegisterAIUpdateEvent },
	{ "ModifyAIUpdateEvent", &luaUnit_ModifyAIUpdateEvent },
	{ "RemoveAIUpdateEvent", &luaUnit_RemoveAIUpdateEvent },
	{ "MonsterMove", &luaUnit_MonsterMove },
	{ "SetPosition", &luaUnit_SetPosition },
	{ "AddAISpell", &luaUnit_AddAISpell },
	{ "AllowMovement", &luaUnit_AllowMovement },
	{ "DisableMovement", &luaUnit_DisableMovement },
...
RegType<GameObject> GOMethods[] = {
	...
	{ "RegisterAIUpdateEvent" ,&luaGameObject_RegisterAIUpdateEvent },
	{ "ModifyAIUpdateEvent", &luaGameObject_ModifyAIUpdateEvent },
	{ "RemoveAIUpdateEvent", &luaGameObject_RemoveAIUpdateEvent },
...
public:
	...
	void OnCombatStart(Unit* mTarget)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_ENTER_COMBAT] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_ENTER_COMBAT], CREATURE_EVENT_ON_ENTER_COMBAT, mTarget, 0,0, 0, false, NULL);
	}

	void OnCombatStop(Unit* mTarget)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_LEAVE_COMBAT] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_LEAVE_COMBAT], CREATURE_EVENT_ON_LEAVE_COMBAT, mTarget, 0, 0, 0, false, NULL );
	}

	void OnTargetDied(Unit* mTarget)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_TARGET_DIED] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_TARGET_DIED], CREATURE_EVENT_ON_TARGET_DIED, mTarget, 0, 0, 0, false, NULL );
	}

	void OnDied(Unit *mKiller)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_DIED] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_DIED], CREATURE_EVENT_ON_DIED, mKiller, 0, 0, 0, false, NULL );
	}
	void OnTargetParried(Unit* mTarget)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_TARGET_PARRIED] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_TARGET_PARRIED], CREATURE_EVENT_ON_TARGET_PARRIED, mTarget, 0, 0, 0, false, NULL );
	}
	void OnTargetDodged(Unit* mTarget)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_TARGET_DODGED] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_TARGET_DODGED], CREATURE_EVENT_ON_TARGET_DODGED, mTarget, 0, 0, 0, false, NULL);
	}
	void OnTargetBlocked(Unit* mTarget, int32 iAmount)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_TARGET_BLOCKED] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_TARGET_BLOCKED], CREATURE_EVENT_ON_TARGET_BLOCKED, mTarget, 0, iAmount, 0, false, NULL );
	}
	void OnTargetCritHit(Unit* mTarget, float fAmount)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_TARGET_CRIT_HIT] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_TARGET_CRIT_HIT], CREATURE_EVENT_ON_TARGET_CRIT_HIT, mTarget, 0, 0, fAmount, false, NULL);
	}
	void OnParried(Unit* mTarget)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_PARRY] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_PARRY], CREATURE_EVENT_ON_PARRY, mTarget, 0, 0, 0, false, NULL);
	}
	void OnDodged(Unit* mTarget)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_DODGED] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_DODGED], CREATURE_EVENT_ON_DODGED, mTarget, 0, 0, 0, false, NULL);
	}
	void OnBlocked(Unit* mTarget, int32 iAmount)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_BLOCKED] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_BLOCKED], CREATURE_EVENT_ON_BLOCKED, mTarget, 0, iAmount, 0, false, NULL);
	}
	void OnCritHit(Unit* mTarget, float fAmount)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_CRIT_HIT] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_CRIT_HIT], CREATURE_EVENT_ON_CRIT_HIT, mTarget, 0, 0, fAmount, false, NULL);
	}
	void OnHit(Unit* mTarget, float fAmount)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_HIT] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_HIT], CREATURE_EVENT_ON_HIT, mTarget, 0, 0, fAmount, false, NULL);
	}
	void OnAssistTargetDied(Unit* mAssistTarget)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_ASSIST_TARGET_DIED] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_ASSIST_TARGET_DIED], CREATURE_EVENT_ON_ASSIST_TARGET_DIED, mAssistTarget, 0, 0, 0, false, NULL);
	}
	void OnFear(Unit* mFeared, uint32 iSpellId)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_FEAR] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_FEAR], CREATURE_EVENT_ON_FEAR, mFeared, iSpellId, 0, 0, false, NULL );
	}
	void OnFlee(Unit* mFlee)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_FLEE] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_FLEE], CREATURE_EVENT_ON_FLEE, mFlee, 0, 0, 0, false, NULL);
	}
	void OnCallForHelp()
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_CALL_FOR_HELP] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_CALL_FOR_HELP], CREATURE_EVENT_ON_CALL_FOR_HELP, NULL, 0, 0, 0, false, NULL);
	}
	void OnLoad()
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_LOAD] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_LOAD], CREATURE_EVENT_ON_LOAD, NULL, 0, 0, 0, false, NULL );
	}
	...
	void OnLootTaken(Player* pPlayer, ItemPrototype *pItemPrototype)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_LOOT_TAKEN] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_LOOT_TAKEN], CREATURE_EVENT_ON_LOOT_TAKEN, pPlayer, 0, 0, 0, false, pItemPrototype );
	}
	void AIUpdate()
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_AIUPDATE] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_AIUPDATE], CREATURE_EVENT_ON_AIUPDATE, NULL, 0, 0, 0, false, NULL );
	}
	void OnEmote(Player * pPlayer, EmoteType Emote)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_EMOTE] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_EMOTE], CREATURE_EVENT_ON_EMOTE, pPlayer, 0, 0, 0, false, NULL);
	}
	...
	void GossipHello(Object* pObject, Player* Plr, bool AutoSend)
	{
		if(pObject->GetTypeId() == TYPEID_UNIT)
        {
           if( m_unit_gossip_binding->Functions[GOSSIP_EVENT_ON_TALK] != NULL )
			    g_engine->OnGossipEvent( pObject, m_unit_gossip_binding->Functions[GOSSIP_EVENT_ON_TALK], GOSSIP_EVENT_ON_TALK, Plr, NULL, 0, NULL );
       }
        else if(pObject->GetTypeId() == TYPEID_ITEM)
        {
            if( m_item_gossip_binding->Functions[GOSSIP_EVENT_ON_TALK] != NULL )
			    g_engine->OnGossipEvent( pObject, m_item_gossip_binding->Functions[GOSSIP_EVENT_ON_TALK], GOSSIP_EVENT_ON_TALK, Plr, NULL, 0, NULL );
        }
	}
	...
	void GossipEnd(Object* pObject, Player* Plr)
	{
		if(pObject->GetTypeId() == TYPEID_UNIT)
        {
		    if( m_unit_gossip_binding->Functions[GOSSIP_EVENT_ON_END] != NULL )
			    g_engine->OnGossipEvent( pObject, m_unit_gossip_binding->Functions[GOSSIP_EVENT_ON_END], GOSSIP_EVENT_ON_END, Plr, NULL, 0, NULL );
        }
        else if(pObject->GetTypeId() == TYPEID_ITEM)
        {
            if( m_item_gossip_binding->Functions[GOSSIP_EVENT_ON_END] != NULL )
			    g_engine->OnGossipEvent( pObject, m_item_gossip_binding->Functions[GOSSIP_EVENT_ON_END], GOSSIP_EVENT_ON_END, Plr, NULL, 0, NULL );
        }
        else if(pObject->GetTypeId() == TYPEID_GAMEOBJECT)
        {
            if( m_go_gossip_binding->Functions[GOSSIP_EVENT_ON_END] != NULL )
			    g_engine->OnGossipEvent( pObject, m_go_gossip_binding->Functions[GOSSIP_EVENT_ON_END], GOSSIP_EVENT_ON_END, Plr, NULL, 0, NULL );
        }
	}
...
int luaUnit_PlayerCastSpell(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_PLAYER);
	uint32 sp = luaL_checkint(L,1);
	if(!sp) return 0;
...
int luaUnit_PlayerCastSpellOnTarget(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_PLAYER);
	uint32 sp = luaL_checkint(L,1);
	Unit * obj = Lunar<Unit>::check(L,2);
	if(!sp || !obj) return 0;
	Player * plr = ((Player*)ptr);
	Spell *spp = new Spell(plr,dbcSpell.LookupEntry(sp),true,0);
	SpellCastTargets tar;
	tar.m_target = obj;
	spp->prepare(&tar);
	return 1;
}
...
int luaUnit_SpawnCreature(lua_State * L, Unit * ptr)
{
	...
	if(duration)
	{
		sEventMgr.AddEvent(pCreature,&Creature::SafeDelete,EVENT_CREATURE_UPDATE,duration,1,0);
	}
...
int luaUnit_SpawnGameObject(lua_State * L, Unit * ptr)
{
	...
	if(entry_id)
	{
		...
		if(duration && duration > 0)
			sEventMgr.AddEvent(pC,&GameObject::ExpireAndDelete,EVENT_GAMEOBJECT_UPDATE,duration,1,0);
...
int luaUnit_Despawn(lua_State * L, Unit * ptr)
{
	...
	Creature * pC = ptr->GetMapMgr()->GetInterface()->SpawnCreature(ptr->GetEntry(),ptr->GetPositionX(),ptr->GetPositionY(),ptr->GetPositionZ(),ptr->GetOrientation(),false,false,0,0);
	pC->SetInstanceID(ptr->GetInstanceID());
	pC->SetMapId(ptr->GetMapId());
	pC->SetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE,ptr->GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE));
	pC->_setFaction();
	MapMgr * mMgr = ptr->GetMapMgr();
	sEventMgr.AddEvent( ((Creature*)ptr),&Creature::SafeDelete,EVENT_CREATURE_RESPAWN,delay,1,0);
	if(respawntime)
	{
		sEventMgr.AddEvent(pC,&Creature::AddToWorld,mMgr,EVENT_CREATURE_RESPAWN,respawntime,1,0);
	}
	return 0;
}
...
{
	if(ptr && ptr->GetTypeId() == TYPEID_UNIT || ptr->GetTypeId() == TYPEID_PLAYER)
		ptr->Root();
	return 0;
}
...
int luaUnit_GetCurrentSpell(lua_State * L, Unit * ptr)
{
	if(ptr != NULL)
		...
		{
			lua_pushlstring(L, ptr->GetCurrentSpell()->m_spellInfo->Name, 0);
		}
	return 1;
}
...
int luaUnit_IsInvisible(lua_State * L, Unit * ptr)
{
	...
	uint32 invisflag = luaL_checkint(L,2);
	if(!choice) return 0;
	if( choice > 0 && invisflag)
	{
		ptr->m_invisFlag = invisflag;
...
int luaUnit_MonsterMove(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_UNIT);
	float x = (float)luaL_checkint(L,1);
	float y = (float)luaL_checkint(L,2);
...
int luaUnit_AddAISpell(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_UNIT);
	uint32 spellid = luaL_checkint(L,1);
...
int luaUnit_RegisterAIUpdateEvent(lua_State * L, Unit * ptr)
{
	...
	if(!time) return 0;
	sEventMgr.AddEvent(((Creature*)ptr),&Creature::CallScriptUpdate,EVENT_CREATURE_UPDATE,time,0,0);
	return 0;
}
int luaUnit_ModifyAIUpdateEvent(lua_State * L, Unit * ptr)
{
	...
	if(!newtime) return 0;
	sEventMgr.ModifyEventTime(((Creature*)ptr),EVENT_CREATURE_UPDATE,newtime);
	return 0;
}
int luaUnit_RemoveAIUpdateEvent(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_UNIT);
	sEventMgr.RemoveEvents( ((Creature*)ptr),EVENT_CREATURE_UPDATE);
	return 0;
}
int luaUnit_AllowMovement(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_UNIT);
	ptr->GetAIInterface()->m_canMove = true;
	return 0;
}
int luaUnit_DisableMovement(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_UNIT);
	ptr->GetAIInterface()->m_canMove = false;
	return 0;
}
///////////////////////////////////////////////
///////////////////////////////////////////////
...
int luaGameObject_Despawn(lua_State * L, GameObject * ptr)
{
	...
	GameObject * pC = ptr->GetMapMgr()->CreateGameObject(ptr->GetEntry());
	pC->m_spawn = 0;
	pC->CreateFromProto(ptr->GetEntry(),ptr->GetMapId(),ptr->GetPositionX(),ptr->GetPositionY(),ptr->GetPositionZ(),ptr->GetOrientation());
	pC->SetMapId(ptr->GetMapId());
	pC->SetInstanceID(ptr->GetInstanceID());
	MapMgr * mgr = ptr->GetMapMgr();
	sEventMgr.AddEvent(ptr,&GameObject::ExpireAndDelete,EVENT_GAMEOBJECT_EXPIRE,delay,1,0);
	if(respawnt)
	{
		sEventMgr.AddEvent(pC,&GameObject::Spawn,mgr,EVENT_GAMEOBJECT_EXPIRE,respawnt,1,0);
	}
	else
		delete pC;
	return 0;
}
int luaGameObject_RegisterAIUpdateEvent(lua_State * L, GameObject * ptr)
{
	CHECK_TYPEID(TYPEID_GAMEOBJECT);
	uint32 time = luaL_checkint(L,1);
	if(!time) return 0;
	sEventMgr.AddEvent(ptr,&GameObject::CallScriptUpdate,EVENT_GAMEOBJECT_UPDATE,time,0,0);
	return 0;
}
int luaGameObject_ModifyAIUpdateEvent(lua_State * L, GameObject * ptr)
{
	CHECK_TYPEID(TYPEID_GAMEOBJECT);
	uint32 ntime = luaL_checkint(L,1);
	if(!ntime) return 0;
	sEventMgr.ModifyEventTime(ptr,EVENT_GAMEOBJECT_UPDATE,ntime);
	return 0;
}
int luaGameObject_RemoveAIUpdateEvent(lua_State * L, GameObject * ptr)
{
	CHECK_TYPEID(TYPEID_GAMEOBJECT);
	sEventMgr.RemoveEvents(ptr,EVENT_GAMEOBJECT_UPDATE);
	return 0;
}
