...
public:
	...
	void OnCast()
	{
		...
		if (_spell->u_caster->HasAura(34692))
		{
			Spell* sp=new Spell(_spell->u_caster, dbcSpell.LookupEntry(34471), true, NULL);
			SpellCastTargets t(_spell->u_caster);
			sp->prepare(&t);
		}
	}
};
...
void SetupHunterSpells(ScriptMgr * mgr)
{
	...
	mgr->register_spell_script(19574, &BestialWrath::Create);
}
