...
public:
	...
	SpellCastError CanCast(bool tolerate)
	{
		...
		if (_spell->u_caster == NULL || u_target == NULL || int32(u_target->getLevel()) > _spell->m_spellInfo->EffectBasePoints[0]+1 + int32(_spell->u_caster->getLevel() - _spell->m_spellInfo->spellLevel))
