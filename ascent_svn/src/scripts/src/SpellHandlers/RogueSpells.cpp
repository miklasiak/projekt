...
public:
	...
	void DummyMeleeEffect(uint32 EffectIndex)
	{
		...
		_spell->p_caster->AddComboPoints(u_target->GetGUID(), 1);
	}
};
...
void SetupRogueSpells(ScriptMgr * mgr)
{
	...
	mgr->register_spell_script(16511, &Hemorrhage::Create);
	mgr->register_spell_script(17347, &Hemorrhage::Create);
	mgr->register_spell_script(17348, &Hemorrhage::Create);
	mgr->register_spell_script(26864, &Hemorrhage::Create);
}
