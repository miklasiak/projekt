...
public:
	...
	void DummyMeleeEffect(uint32 EffectIndex)
	{
		...
		if (u_target == NULL || _spell->p_caster == NULL)
			return;

		//count the number of sunder armors on target
		uint32 sunder_count=0;
		SpellEntry* sp=dbcSpell.LookupEntry(_spell->p_caster->GetMaxRankByHash(SPELL_HASH_SUNDER_ARMOR));
		...
		Spell *spell = new Spell(_spell->p_caster, sp, true, NULL);
...
void SetupWarriorSpells(ScriptMgr * mgr)
{
    ...
    */
	...
	mgr->register_spell_script(20243, &Devastate::Create);
	mgr->register_spell_script(30016, &Devastate::Create);
	mgr->register_spell_script(30022, &Devastate::Create);
}
