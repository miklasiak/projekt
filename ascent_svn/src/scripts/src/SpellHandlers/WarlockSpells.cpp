...
public:
	...
	SpellCastError CanCast(bool tolerate)
	{
		...
		if (_spell->u_caster->GetUInt32Value(UNIT_FIELD_HEALTH) <= (uint32)_spell->CalculateEffect(0, _spell->u_caster))
...
public:
	...
	void CalculateEffect(uint32 EffectIndex, Unit* target, int32* value)
	{
		if (_spell->u_caster == NULL || !_spell->u_caster->IsPet() || static_cast<Pet*>(_spell->u_caster)->GetOwner() == NULL)
			return;
...
void SetupWarlockSpells(ScriptMgr * mgr)
{
	...
	mgr->register_spell_script(34947, &WarlockPetScaling01::Create);
}
