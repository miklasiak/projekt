...
#define SET_SCHOOL(x) sp->School = x
		if(sp->schoolMask & 1)
			SET_SCHOOL(SCHOOL_NORMAL);
		else if(sp->schoolMask & 2)
			SET_SCHOOL(SCHOOL_HOLY);
		else if(sp->schoolMask & 4)
			SET_SCHOOL(SCHOOL_FIRE);
		else if(sp->schoolMask & 8)
			SET_SCHOOL(SCHOOL_NATURE);
		else if(sp->schoolMask & 16)
			SET_SCHOOL(SCHOOL_FROST);
		else if(sp->schoolMask & 32)
			SET_SCHOOL(SCHOOL_SHADOW);
...
#undef SET_SCHOOL
		...
		*/
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////
		// Paladin Blessing types
		//////////////////////////////////////////////////////////////////////////////////////////////////////

		if (strstr(sp->Name, "of Freedom"))
			sp->UniqueGroup2 = 1;
		if (strstr(sp->Name, "of Kings"))
			sp->UniqueGroup2 = 2;
		if (strstr(sp->Name, "of Light"))
			sp->UniqueGroup2 = 3;
		if (strstr(sp->Name, "of Might"))
			sp->UniqueGroup2 = 4;
		if (strstr(sp->Name, "of Protection"))
			sp->UniqueGroup2 = 5;
		if (strstr(sp->Name, "of Sacrifice"))
			sp->UniqueGroup2 = 6;
		if (strstr(sp->Name, "of Salvation"))
			sp->UniqueGroup2 = 7;
		if (strstr(sp->Name, "of Sanctuary"))
			sp->UniqueGroup2 = 8;
		if (strstr(sp->Name, "of Wisdom"))
			sp->UniqueGroup2 = 9;
		...
		//seal of command
			...
			sp->defenseType = DEFENSE_TYPE_MAGIC;
...
/*		//if there is a proc spell and has 0 as charges then it's probably going to triger infinite times. Better not save these
		...
		if( sp->NameHash == SPELL_HASH_SEAL_OF_COMMAND )
		{
			...
			sp->defenseType = DEFENSE_TYPE_MAGIC; //heh, crazy spell uses melee/ranged/magic dmg type for 1 spell. Now which one is correct ?
		}
...
//	printf("!!!!!!! name %s , id %u , hash %u \n",nametext,sp->Id, namehash);
	...
	if( sp != NULL )
		sp->defenseType = DEFENSE_TYPE_RANGED;
	...
	// Insert warrior spell fixes here
		...
		sp = dbcSpell.LookupEntryForced( 772 );
		if( sp != NULL )
			sp->mechanics = MECHANIC_BLEEDING;
		sp = dbcSpell.LookupEntryForced( 6546 );
		if( sp != NULL )
			sp->mechanics = MECHANIC_BLEEDING;
		sp = dbcSpell.LookupEntryForced( 6547 );
	...
	// Insert warlock spell fixes here
		...
		sp = dbcSpell.LookupEntryForced(30327);
		if (sp != NULL)
		{
			sp->EffectApplyAuraName[0] = SPELL_AURA_ADD_PCT_MODIFIER;
		}
		sp = dbcSpell.LookupEntryForced(30328);
		if (sp != NULL)
		{
			sp->EffectApplyAuraName[0] = SPELL_AURA_ADD_PCT_MODIFIER;
		}
	...
	// Insert items spell fixes here
		...
		if( sp != NULL )
		{
			sp->Flags5 = ATTRIBUTESEXD_PROCCHANCE_COMBOBASED;
		}
	#endif
		//Extract Gas 
		...
		// Mark of Conquest: reduced proc rate
		...
		// Insert boss spell fixes here
			...
			// Soul Flay
			sp = dbcSpell.LookupEntryForced( 45442 );
			if (sp != NULL)
				sp->channelInterruptFlags |= 1;

			// Shadow Spike
			sp = dbcSpell.LookupEntryForced( 46680 );
			if (sp != NULL)
				sp->channelInterruptFlags |= 1;

			// Darkness of a Thousand Souls
			sp = dbcSpell.LookupEntryForced( 46605 );
			if (sp != NULL)
				sp->channelInterruptFlags |= 1;
