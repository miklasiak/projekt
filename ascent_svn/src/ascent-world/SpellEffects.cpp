...
pSpellEffect SpellEffectsHandler[TOTAL_SPELL_EFFECTS]={
		...
		&Spell::SpellEffectNULL,//SPELL_EFFECT_SUMMON_TOTEM - 74
		...
		&Spell::SpellEffectNULL,// unknown - 130 // redirect threat
		...
		&Spell::SpellEffectNULL,// unknown - 134 // give quest credit
		&Spell::SpellEffectNULL,// unknown - 135 // npc's call pet o.o
		&Spell::SpellEffectNULL,// unknown - 136 // restore health pct
		&Spell::SpellEffectNULL,// unknown - 137 // restore mana pct
		...
		&Spell::SpellEffectTriggerSpell,// this forces a spell to cast, why? who knows
		...
		&Spell::SpellEffectNULL,// unknown - 143 // apply aura to summoned demon and owner
		&Spell::SpellEffectNULL,// unknown - 144 // unknown
		&Spell::SpellEffectNULL,//145 - modify gravity? o.o
		&Spell::SpellEffectNULL,//146 - refresh rune
		&Spell::SpellEffectNULL,//147 - fail quest
		//148 - damage? lol wtf
		//149 - charge to position (target or radius)
		//150 - unknown
		//151 - summon target (used by ritual of summon, wtf?)
		//152 - summon refer-a-friend
		//153 - create tamed creature
		//154 - unknown
		//155 - allow offhand wield (used for titans grip twice, once for 2h weapons, once for shields)
		//156 - add socket
		//157 - create item (inscription)
		//158 - milling
		//159 - allow hunter pet rename
};
...
void Spell::SpellEffectSchoolDMG(uint32 i) // dmg school
{
	...
	if(m_spellInfo->SpellGroupType && u_caster)
		SM_FFValue(u_caster->SM_PJumpReduce, &reduce, m_spellInfo);
	...
	if (m_spellInfo->EffectChainTarget == 0 && dmg * m_orderedObjects.size() > ((uint32)m_spellInfo->EffectBasePoints[i] + 1) * 15)
		dmg = (m_spellInfo->EffectBasePoints[i] + 1) * 15 / m_orderedObjects.size();
...
void Spell::SpellEffectApplyAura(uint32 i)  // Apply Aura
{
	...
	if (m_auraTargets.find(unitTarget) != m_auraTargets.end())
		return;
	...
	if (m_spellInfo->channelInterruptFlags != 0 && u_caster != NULL)
		Duration *= u_caster->GetFloatValue(UNIT_MOD_CAST_SPEED);
	...
	if (m_spellInfo->UniqueGroup != 0)
	{
		...
		{
			std::multimap<uint32, Aura*>::iterator itr = u_caster->m_aurascast.find(m_spellInfo->UniqueGroup);
			if (itr != u_caster->m_aurascast.end())
			{
	...
	else if (m_spellInfo->cumulativeAura == 0)
		oldaura=unitTarget->FindAura(m_spellInfo->Id, caster->GetGUID());
	else
		oldaura=unitTarget->FindAura(m_spellInfo->Id); //this aura IS UNIQUE

	if (oldaura != NULL)
	{
		oldaura->SetDuration(Duration);
		oldaura->HandleStackAdd();
		m_auraTargets.insert(unitTarget);
		return;
	}

	SpellTargetMap::iterator sitr;
	if (m_backupTarget != NULL) //this spell was reflected :D
	{
		pAura = new Aura(m_spellInfo, Duration, m_backupTarget, unitTarget);
		sitr = m_spellTargets.find(m_backupTarget);
	}
	else
	{
		pAura=new Aura(m_spellInfo, Duration, caster, unitTarget);
		sitr = m_spellTargets.find(unitTarget);
	}

	pAura->pSpellId = pSpellId;

	bool hasmodifier = false;
	bool boss = false;

	//since we apply once, apply all modifiers :)
	for (uint32 j=0; j<3; ++j)
	{
		//SpellTargetMap::iterator sitr = m_spellTargets.find(unitTarget);
		if (sitr == m_spellTargets.end() || !sitr->second.HasEffect[j])
			continue;

		//this index isnt an aura application :P
		if (m_spellInfo->EffectApplyAuraName[j] == 0)
			continue;
...
void Spell::SpellEffectWeapondamageNoschool(uint32 i) // Weapon damage + (no School)
{
	...
	u_caster->Strike( unitTarget, ( GetType() == DEFENSE_TYPE_RANGED ? RANGED : MELEE ), m_spellInfo, damage, 0, 0, false, true );
}
...
void Spell::SpellEffectPersistentAA(uint32 i) // Persistent Area Aura
{
	...
	if(g_caster && g_caster->m_summoner && !unitTarget)
	{
		...
		if (g_caster->GetByte(GAMEOBJECT_BYTES_1, 1) == GAMEOBJECT_TYPE_TRAP && g_caster->m_summoner->dynObj != NULL)
			g_caster->m_summoner->dynObj->m_aliveDuration = 1;
	...
	case TARGET_FLAG_SELF:
		{
			dynObj->Create(u_caster? u_caster : g_caster->m_summoner, this,	m_caster->GetPositionX(), 
	...
	case TARGET_FLAG_UNIT:
		{
			if(!unitTarget||!unitTarget->isAlive())
				break;
			dynObj->Create(u_caster? u_caster : g_caster->m_summoner, this, unitTarget->GetPositionX(), unitTarget->GetPositionY(), unitTarget->GetPositionZ(),
				dur, r);
		}break;
	...
	case TARGET_FLAG_SOURCE_LOCATION:
		{
			dynObj->Create(u_caster? u_caster : g_caster->m_summoner, this, m_targets.m_srcX,
	...
	if(u_caster != NULL && m_spellInfo->channelInterruptFlags > 0)
	{
...
void Spell::SpellEffectSummon(uint32 i) // Summon
{
	...
	if (u_caster->m_summonslot[props->slot] != 0 && props->slot != 0) //slot 0 can have infinite of them up
		u_caster->m_summonslot[props->slot]->SafeDelete();
	...
	for (int32 amount=0; amount<summonamount; ++amount)
	{
		...
		float x=m_targets.m_destX, y=m_targets.m_destY, z=m_targets.m_destZ;
		...
		if (props->controltype == 2) //pets need rewrote, this is SHIT
		{
			summon=objmgr.CreatePet();
			summon->SetInstanceID(u_caster->GetInstanceID());
			static_cast<Pet*>(summon)->CreateAsSummon(m_spellInfo->EffectMiscValue[i], ci, NULL, u_caster, m_spellInfo, 1, GetDuration());
			u_caster->SetUInt64Value(UNIT_FIELD_SUMMON, summon->GetGUID());
		}
		else
		{
			summon=u_caster->GetMapMgr()->CreateCreature(cp->Id);
			summon->Load(cp, x, y, z);
		}
		...
		{
			//factions
			summon->SetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE, props->factionId? props->factionId : u_caster->GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE));
			summon->_setFaction();

			summon->GetAIInterface()->SetUnitToFollow(u_caster);
			summon->GetAIInterface()->SetUnitToFollowAngle(float(-(M_PI/2)));
			summon->GetAIInterface()->SetFollowDistance(3.0f);
			summon->SetUInt64Value(UNIT_FIELD_SUMMONEDBY, u_caster->GetGUID());
			summon->SetUInt64Value(UNIT_FIELD_CREATEDBY, u_caster->GetGUID());
			summon->SetUInt32Value(UNIT_FIELD_FLAGS, u_caster->GetUInt32Value(UNIT_FIELD_FLAGS));
			summon->SetSummonOwner(u_caster);
			summon->SetSummonSlot(props->slot);
			u_caster->m_summonslot[props->slot] = summon;
		}
		...
		{
			summon->GetAIInterface()->Init(summon, AITYPE_PET, MOVEMENTTYPE_NONE, u_caster);
			u_caster->m_guardians.insert(summon);

			//we now have to fix every other angle ROFL
			if (u_caster->m_guardians.size() > 0)
			{
				float angle_for_each_spawn = -float(M_PI) * 2 / u_caster->m_guardians.size();
				int i=0;
				for (std::set<Creature*>::iterator itr=u_caster->m_guardians.begin(); itr!=u_caster->m_guardians.end(); ++itr, ++i)
				{
					(*itr)->GetAIInterface()->SetUnitToFollow(u_caster);
		...
		if (props->type == SUMMON_TYPE_TOTEM)
		{
			...
			if (p_caster != NULL)
			{
				...
				*data << GetDuration();
				*data << m_spellInfo->Id;
				p_caster->delayedPackets.add(data);
			}
			
			
			uint32 displayID = 0;

			if( p_caster != NULL && p_caster->GetTeam() == 0 )
			{
			...
			summon->SetUInt32Value(UNIT_FIELD_POWER3, u_caster->getLevel() * 30);
			summon->SetUInt32Value(UNIT_FIELD_MAXPOWER3, u_caster->getLevel() * 30);
			...
			else
			{
				...
				summon->EnableAI();
				summon->GetAIInterface()->Init(summon, AITYPE_TOTEM, MOVEMENTTYPE_NONE, u_caster);
		...
		{
			Spell* s=new Spell(u_caster, dbcSpell.LookupEntry(11726), true, NULL);
...
void Spell::SpellEffectLeap(uint32 i) // Leap
{
	...
	for(uint32 i = MAX_POSITIVE_AURAS; i < MAX_AURAS; ++i)
	{
		if( u_caster->m_auras[i] != NULL )
		{
			for(uint32 j = 0; j < 3; ++j)
			{
				if( u_caster->m_auras[i]->GetSpellProto()->EffectApplyAuraName[j] == SPELL_AURA_MOD_STUN || 
					u_caster->m_auras[i]->GetSpellProto()->EffectApplyAuraName[j] == SPELL_AURA_MOD_ROOT )
				{
					u_caster->m_auras[i]->Remove();
					break;
				}
			}
		}
	...
	float ori = u_caster->GetOrientation();				
	float posX = u_caster->GetPositionX()+(radius*(cosf(ori)));
	float posY = u_caster->GetPositionY()+(radius*(sinf(ori)));
	float z = CollideInterface.GetHeight(u_caster->GetMapId(), posX, posY, u_caster->GetPositionZ() + 2.0f);
	if(z == NO_WMO_HEIGHT)		// not found height, or on adt
		z = u_caster->GetMapMgr()->GetLandHeight(posX,posY);

	if( fabs( z - u_caster->GetPositionZ() ) >= 10.0f )
		return;

	LocationVector dest(posX, posY, z + 2.0f, ori);
	LocationVector destest(posX, posY, dest.z, ori);
	LocationVector src(u_caster->GetPositionX(), u_caster->GetPositionY(), u_caster->GetPositionZ() + 2.0f);

	if(CollideInterface.GetFirstPoint(u_caster->GetMapId(), src, destest, dest, -1.5f))
	{
	...
	{
		u_caster->SetPosition(dest, false);
	}
}
...
void Spell::SpellEffectWeaponDmgPerc(uint32 i) // Weapon Percent damage
{
	...
	if( GetType() == DEFENSE_TYPE_MAGIC )
	{
		float fdmg = (float)CalculateDamage( u_caster, unitTarget, MELEE, m_spellInfo );
...
void Spell::SpellEffectApplyAA(uint32 i) // Apply Area Aura
{
	...
	if(itr==unitTarget->tmpAura.end())
	{
		...
		if(!sEventMgr.HasEvent(pAura, EVENT_AREAAURA_UPDATE))		/* only add it once */
		{
			...
			sEventMgr.AddEvent(pAura, &Aura::EventUpdateAA, r*r, EVENT_AREAAURA_UPDATE, 1, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);	
		}
	}
	else 
	{
...
void Spell::SpellEffectSummonObject(uint32 i)
{
	...
	else
	{
		...
		if (!CollideInterface.CheckLOS(m_caster->GetMapId(), m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ() + 2.0f, posx, posy, m_caster->GetPositionZ() + 2.0f))
		{
			posx = m_caster->GetPositionX();
			posy = m_caster->GetPositionY();
		}
...
void Spell::SpellEffectSummonPet(uint32 i) //summon - pet
{
	if(m_spellInfo->Id == 883)
	{
		...
		if(petno)
		{
			...
			Pet* summon = p_caster->GetSummon();

			if (summon == NULL)
				return;

			SpellCastTargets t(summon);
			Spell* s=new Spell(summon, dbcSpell.LookupEntry(34902), true, NULL);
			Spell* s2=new Spell(summon, dbcSpell.LookupEntry(34903), true, NULL);
			Spell* s3=new Spell(summon, dbcSpell.LookupEntry(34904), true, NULL);
			s->prepare(&t);
			s2->prepare(&t);
			s3->prepare(&t);
		}
	...
	if(ci)
	{
		...
		u_caster->RemoveAura(18789);
		u_caster->RemoveAura(18790);
		u_caster->RemoveAura(18791);
		u_caster->RemoveAura(18792);
		u_caster->RemoveAura(35701);
...
void Spell::SpellEffectDispelMechanic(uint32 i)
{
	...
	for( uint32 x = 0 ; x<MAX_AURAS ; x++ )
	{
		...
		{
			if( unitTarget->m_auras[x]->GetSpellProto()->mechanics == sMisc )
				unitTarget->m_auras[x]->Remove();
		}
	}
	*/
...
void Spell::SpellEffectSummonDemon(uint32 i)
{
	...
	if(ci != NULL)
	{
		...
		pPet->SetInstanceID(u_caster->GetInstanceID());
		pPet->CreateAsSummon(m_spellInfo->EffectMiscValue[i], ci, NULL, u_caster, m_spellInfo, 1, GetDuration(), this);
	}
}
	...
	if(unitTarget->m_auras[x] != NULL)
	{
		...
		if(aur->GetSpellId() != 15007 && !aur->IsPassive() && aur->IsPositive()) //Nothing can dispel resurrection sickness
		{
			if(aur->GetSpellProto()->dispelType == DISPEL_MAGIC)
			{
				...
				aura->SetTimeLeft(aur->GetTimeLeft());
...
void Spell::SpellEffectEnchantHeldItem( uint32 i )
{
	...
	switch( m_spellInfo->NameHash )
	{
		case SPELL_HASH_FLAMETONGUE_TOTEM_EFFECT:
