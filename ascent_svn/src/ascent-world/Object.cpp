...
void Object::_setFaction()
{
	...
	if (IsUnit())
	{
		...
		{
			if ((*i)->IsUnit() && isFriendly(this, u))
			{
				static_cast<Unit*>(*i)->GetAIInterface()->RemoveThreatByPtr(u);

				if (IsCreature())
					u->GetAIInterface()->RemoveThreatByPtr(static_cast<Unit*>(*i));

				//casting a spell? not anymore!
				if (static_cast<Unit*>(*i)->GetCurrentSpell() && static_cast<Unit*>(*i)->GetCurrentSpell()->GetUnitTarget() && static_cast<Unit*>(*i)->GetCurrentSpell()->GetUnitTarget() == this)
					sEventMgr.AddEvent(static_cast<Unit*>(*i)->GetCurrentSpell(), &Spell::cancel, EVENT_UNK, 1, 1, 0);
			}
		}
	}
	//need this for new aggro rules (new faction)
	sEventMgr.AddEvent(this, &Object::UpdateOppFactionSet, EVENT_UNK, 1, 1, 0);
}
...
/*		if(!pVictim->isInCombat() && pVictim->IsPlayer())
	...
	else /* ---------- NOT DEAD YET --------- */
	{
		...
		for (; itr != itrend; ++itr)
		{
			...
			if (health > damage)
				(*itr)->SetUInt32Value( UNIT_FIELD_HEALTH, health - damage );
			else
				(*itr)->SetUInt32Value( UNIT_FIELD_HEALTH, 1 );
		}
...
{
	if (IsCreature() && static_cast<Creature*>(this)->IsTotem())
	{
		static_cast<Creature*>(this)->SafeDelete();
		return;
	}
