...
void WorldSession::HandleSplitOpcode(WorldPacket& recv_data)
{
	...
	if (DstSlot >= INVENTORY_KEYRING_START && DstSlot < INVENTORY_KEYRING_END)
	{
		_player->GetItemInterface()->BuildInventoryChangeError(i1, i2, INV_ERR_ITEM_DOESNT_GO_INTO_BAG);
		return;
	}
...
//	int8 bagslot = 0;
   ...
   if((error = _player->GetItemInterface()->CanAffordItem(it, amount, unit)))
   {
	...
	if(add == NULL || add->wrapped_item_id != 0)
	{
		...
		if (amount * item.amount > itm->GetProto()->MaxCount)
			itm->SetUInt32Value(ITEM_FIELD_STACK_COUNT, itm->GetProto()->MaxCount);
		else
	...
	for(std::vector<CreatureItem>::iterator itr = unit->GetSellItemBegin(); itr != unit->GetSellItemEnd(); ++itr)
	{
		if(itr->itemid && (itr->max_amount == 0 || (itr->max_amount>0 && itr->available_amount >0)))
		{
			if((curItem = ItemPrototypeStorage.LookupEntry(itr->itemid)))
			{
				...
				else if( curItem->extended_cost != NULL )
