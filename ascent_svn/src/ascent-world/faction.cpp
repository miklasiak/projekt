...
bool isHostile(Object* objA, Object* objB)// B is hostile for A?
{
	...
	if( hostile && ( objA->IsPlayer() || objA->IsPet() || ( objA->IsUnit() && !objA->IsPlayer() && static_cast< Creature* >( objA )->IsTotem() && static_cast< Creature* >( objA )->GetSummonOwner()->IsPlayer() && static_cast<Player*>(static_cast< Creature* >( objA )->GetSummonOwner())->IsPvPFlagged() ) ) )
	{
		...
		if( objB->IsPet() )
		{
			...
			__try {
				// Check PvP Flags.
				if( static_cast< Pet* >( objB )->GetPetOwner() != NULL && static_cast< Pet* >( objB )->GetPetOwner()->IsPlayer() && static_cast< Pet* >( objB )->GetPetOwner()->GetMapMgr() == objB->GetMapMgr() && static_cast<Player*>(static_cast< Pet* >( objB )->GetPetOwner())->IsPvPFlagged() )
					return true;
				else
					return false;
	...
	if(objA->IsPlayer() && !objB->IsPlayer() && static_cast<Player*>(objA)->GetAIInterface()->MindControlOwner == NULL)	   // PvE
	{
