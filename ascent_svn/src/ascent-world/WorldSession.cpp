...
void WorldSession::LogoutPlayer(bool Save)
{
	...
	if( _player != NULL )
	{
		...
		//remove guardians!
		for (std::set<Creature*>::iterator itr=_player->m_guardians.begin(); itr!=_player->m_guardians.end(); ++itr)
			(*itr)->SafeDelete();
		_player->m_guardians.clear();

		for (std::multimap<uint32, Aura*>::iterator itr=_player->m_aurascast.begin(); itr!=_player->m_aurascast.end(); ++itr)
		{
			if (itr->second->m_spellProto->UniqueTargetBased & 2) //1 target at a time
				sEventMgr.AddEvent(itr->second, &Aura::Remove, true, EVENT_AURA_REMOVE, 1000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT | EVENT_FLAG_DELETES_OBJECT);
		}
		//needs to be clear here, we can't have dead pointers later in Unit::~Unit
		_player->m_aurascast.clear();

		sHookInterface.OnLogout( pPlayer );
		if( pPlayer->DuelingWith )
			pPlayer->EndDuel( DUEL_WINNER_RETREAT );
...
void WorldSession::InitPacketHandlerTable()
{
	...
	WorldPacketHandlers[CMSG_VOICE_CHAT_STATUS].handler = &WorldSession::HandleVoiceChatQueryOpcode;
	WorldPacketHandlers[CMSG_CHANNEL_VOICE_STATUS].handler = &WorldSession::HandleChannelVoiceQueryOpcode;
