...
AIInterface::~AIInterface()
{
	...
	for (std::set<uint64>::iterator itr=MindControlChilds.begin(); itr!=MindControlChilds.end(); ++itr)
	{
		...
		u->GetAIInterface()->MindControlAura->Remove(true);
...
//				CancelSpellCast();
				...
				if (MindControlOwner != NULL)
				{
					...
					{
						//if we got here, we have nothing to attack. move to the return position of our owner
						float x=MindControlOwner->GetAIInterface()->m_returnX? MindControlOwner->GetAIInterface()->m_returnX:MindControlOwner->GetSpawnX();
						float y=MindControlOwner->GetAIInterface()->m_returnY? MindControlOwner->GetAIInterface()->m_returnY:MindControlOwner->GetSpawnY();
						float z=MindControlOwner->GetAIInterface()->m_returnZ? MindControlOwner->GetAIInterface()->m_returnZ:MindControlOwner->GetSpawnZ();
		...
		case EVENT_POSSESS:
			{
				...
				if (MindControlOwner->GetAIInterface()->GetAITargets()->size() > 0)
				{
					uint32 index=RandomUInt(MindControlOwner->GetAIInterface()->GetAITargets()->size()-1);
					int i=0;
...
void AIInterface::_UpdateCombat(uint32 p_time)
{
	...
	if (m_nextTarget != NULL && m_nextTarget->m_auracount[SPELL_AURA_IGNORE_ENEMY] > 0 && m_aiTargets.size() > 1)
...
#endif
	if( cansee && m_nextTarget && m_nextTarget->isAlive() && m_AIState != STATE_EVADE && !m_Unit->isCasting() && m_nextTarget->GetAIInterface()->getAIState() != STATE_EVADE )
	{
...
//				m_Unit->SetUInt64Value(UNIT_FIELD_TARGET, 0);
	...
	else if( !m_nextTarget || m_nextTarget->GetInstanceID() != m_Unit->GetInstanceID() || !m_nextTarget->isAlive() || !cansee || m_nextTarget->GetAIInterface()->getAIState() == STATE_EVADE )
	{
		//SetNextTarget( NULL );
		// no more target
		...
		//remove threat
		if (m_nextTarget != NULL)
			RemoveThreatByPtr(m_nextTarget);
		else
			HandleEvent(EVENT_LEAVECOMBAT, m_Unit, 0);
	}
}
...
//			uint8 wt = m_Unit->GetMapMgr()->GetWaterType(Fx, Fy);
			...
			if (CollideInterface.CheckLOS(m_Unit->GetMapId(), m_Unit->GetPositionX(), m_Unit->GetPositionY(), m_Unit->GetPositionZ(), Fx, Fy, Fz))
			{
				MoveTo(Fx, Fy, Fz, Fo);
	...
	if(m_AIState == STATE_WANDER && m_creatureState == STOPPED)
	{
		...
		float wanderZ = m_Unit->GetPositionZ();
