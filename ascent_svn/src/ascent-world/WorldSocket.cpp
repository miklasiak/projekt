...
void WorldSocket::_HandlePing(WorldPacket* recvPacket)
{
	...
	*recvPacket >> _latency;
	...
	if (sWorld.m_nagleswitch > 0)
	{
		if(_latency >= sWorld.m_nagleswitch)
		{
			if(!m_nagleEanbled)
			{
				u_long arg = 0;
	...
	{
		if (sWorld.m_nagleswitch == 0 && m_nagleEanbled)
		{
			u_long arg = 1;
			setsockopt(GetFd(), 0x6, 0x1, (const char*)&arg, sizeof(arg));
			m_nagleEanbled = false;
		...
		if (sWorld.m_nagleswitch == -1 && !m_nagleEanbled)
		{
			u_long arg = 0;
			setsockopt(GetFd(), 0x6, 0x1, (const char*)&arg, sizeof(arg));
			m_nagleEanbled = true;
