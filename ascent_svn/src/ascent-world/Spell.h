...
enum attributesExB
{
	ATTRIBUTESEXB_NULL               = 0x0,
	ATTRIBUTESEXB_UNK2               = 0x1,
	ATTRIBUTESEXB_UNK3               = 0x2,    // Can be used while stealthed
	ATTRIBUTESEXB_UNK4               = 0x4,    // request pet maybe
	ATTRIBUTESEXB_UNK5               = 0x8,    // something todo with temp enchanted items
	ATTRIBUTESEXB_PARTY_EFFECTING_AURA = 0x10, // Party affecting aura's
	ATTRIBUTESEXB_ACTIVATE_AUTO_SHOT = 0x20,   // spell that enable's auto shoot
	ATTRIBUTESEXB_UNK8               = 0x40,   //Polymorph spells
	ATTRIBUTESEXB_UNK9               = 0x80,
	ATTRIBUTESEXB_UNUSED1            = 0x100,
	ATTRIBUTESEXB_UNK11              = 0x200,  // used by 2 spells, 30421 | Nether Portal - Perseverence and  30466 | Nether Portal - Perseverence
	ATTRIBUTESEXB_TAME_X             = 0x400,  // tame [creature]
	ATTRIBUTESEXB_FUNNEL             = 0x800,  // only funnel spells
	ATTRIBUTESEXB_UNK14              = 0x1000, // swipe / Cleave spells
	ATTRIBUTESEXB_ENCHANT_OWN_ONLY   = 0x2000, // no trade window targets, BoE items get soulbound to you
	ATTRIBUTESEXB_SPELL_PLAYER_EVENT = 0x4000, // Player event's like logging in, finishing quests, triggering cinematic, being adored, Heartbroken etc
	ATTRIBUTESEXB_UNUSED3            = 0x8000,
	ATTRIBUTESEXB_CONTROL_UNIT       = 0x10000, // PvP Controller, RC, Creature taming, Taming Lesson
	ATTRIBUTESEXB_REQ_RANGED_WEAPON  = 0x20000, // this is shit and has nothing to do with auto shot
	ATTRIBUTESEXB_REVIVE_PET         = 0x40000, // actually 1 spell, revive pet
	ATTRIBUTESEXB_UNK21              = 0x80000, // this is a group of spells that are triggered by something. (I have no clue on how to name this one)
	ATTRIBUTESEXB_REQ_BEHIND_TARGET  = 0x100000, //wrong
	ATTRIBUTESEXB_UNK23              = 0x200000,
	ATTRIBUTESEXB_UNK24              = 0x400000,
	ATTRIBUTESEXB_UNK25              = 0x800000,
	ATTRIBUTESEXB_UNK26              = 0x1000000,
	ATTRIBUTESEXB_UNK27              = 0x2000000,
	ATTRIBUTESEXB_UNK28              = 0x4000000,
	ATTRIBUTESEXB_UNK29              = 0x8000000, // fishing spells and enchanting weapons
...
enum attributesExC
{
	ATTRIBUTESEXC_BG_ONLY							= 0x800,
	ATTRIBUTESEXC_PLAYER_RANGED_SPELLS				= 0x8000,
	ATTRIBUTESEXC_CAN_PERSIST_AND_CASTED_WHILE_DEAD	= 0x100000,
	ATTRIBUTESEXC_PLAYER_RANGED_WAND				= 0x400000
};

enum attributesExD
{
	ATTRIBUTESEXD_PROCCHANCE_COMBOBASED        = 0x2,
	ATTRIBUTESEXD_ONLY_IN_OUTLANDS             = 0x4000000,
};
...
enum SpellEffects
{
	...
	SPELL_EFFECT_MODIFY_GRAVITY,
	SPELL_EFFECT_REFRESH_RUNE,
	SPELL_EFFECT_FAIL_QUEST,
...
public:
    ...
    SpellCastTargets() : m_targetMask(0), m_targetMaskExtended(0), m_target(0), m_srcX(0), m_srcY(0), m_srcZ(0),
        m_destX(0), m_destY(0), m_destZ(0) {}

    SpellCastTargets(uint16 TargetMask, Object* target, float srcX, float srcY,
        float srcZ, float destX, float destY, float destZ) : m_targetMask(TargetMask), m_targetMaskExtended(0),
        m_target(target), m_srcX(srcX), m_srcY(srcY), m_srcZ(srcZ), m_destX(destX), m_destY(destY), m_destZ(destZ) {}

    SpellCastTargets(Object* target) : m_targetMask(0x2), m_targetMaskExtended(0), m_target(target),
        m_srcX(0), m_srcY(0), m_srcZ(0), m_destX(0), m_destY(0), m_destZ(0) {}

	SpellCastTargets(WorldPacket & data, Object* obj) { read(data, obj); }
...
typedef enum {
   EFF_TARGET_NONE										= 0,
   EFF_TARGET_SELF										= 1,
   EFF_TARGET_INVISIBLE_OR_HIDDEN_ENEMIES_AT_LOCATION_RADIUS		= 3, //this is wrong, its actually a friendly single target
   EFF_TARGET_PET										= 5,
...
public:
	...
	uint32 ProcData; //could possible change for every target from an aoe, for example
	...
	ASCENT_INLINE uint32 GetType() { return ( m_spellInfo->defenseType == DEFENSE_TYPE_NONE ? DEFENSE_TYPE_MAGIC : m_spellInfo->defenseType ); }
	ASCENT_INLINE Item* GetItemTarget() { if (itemTarget == NULL && m_targets.m_target != NULL && m_targets.m_target->GetTypeId() == TYPEID_ITEM) return static_cast<Item*>(m_targets.m_target); else return itemTarget; }
	ASCENT_INLINE Unit* GetUnitTarget() { if (unitTarget == NULL && m_targets.m_target != NULL && m_targets.m_target->IsUnit()) return static_cast<Unit*>(m_targets.m_target); else return unitTarget; }
    ...
    uint32 GetDuration()
    {
        ...
        if(m_spellInfo->durationIndex)
        {
            ...
            if(sd)
            {
                ...
                {
                    SM_FIValue(u_caster->SM_FDur,(int32*)&this->Dur,m_spellInfo);
                    SM_PIValue(u_caster->SM_PDur,(int32*)&this->Dur,m_spellInfo);
	...
	ASCENT_INLINE float GetRadius(uint32 i)
    {
        ...
        {
            SM_FFValue(u_caster->SM_FRadius,&Rad[i],m_spellInfo);
            SM_PFValue(u_caster->SM_PRadius,&Rad[i],m_spellInfo);
#ifdef COLLECTION_OF_UNTESTED_STUFF_AND_TESTERS
