...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
	...
	{
		sCheatLog.writefromsession(this, "player tried to cast a passive spell (spell id %u, player guid "I64FMT")", spellInfo->Id, GetPlayer()->GetGUID());
		return;
	}
	...
	if (GetPlayer()->GetOnMeleeSpell() != spellId)
	{
		...
		if( spellInfo->EffectImplicitTargetA[0] == 1 )
		{
			if( targets.m_target && targets.m_target != _player )
			{
				// send the error message
				_player->SendCastResult(spellInfo->Id, SPELL_FAILED_BAD_TARGETS, cn, 0);
				return;
