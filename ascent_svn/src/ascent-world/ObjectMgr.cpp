...
void ObjectMgr::LoadVendors()
{
	...
	if( result != NULL )
	{
		...
		do
		{
			...
			itm.extcost			 = fields[5].GetUInt32();

			if (itm.extcost != 0)
				itm.extended_cost = dbcItemExtendedCost.LookupEntryForced(itm.extcost);
			else
...
{
	uint32 guid;
	guid = Sync_Add((volatile long*)&m_hiPetGuid);
	return new Pet((uint64)HIGHGUID_TYPE_PET<<32 | guid);
}

Player * ObjectMgr::CreatePlayer()
{
	return new Player(Sync_Add((volatile long*)&m_hiPlayerGuid));
}
