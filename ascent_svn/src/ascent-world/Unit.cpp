...
Unit::~Unit()
{
	...
	if (!m_modifiersInherited)
	{
		...
		if(SM_FAmptitude != 0) delete [] SM_FAmptitude ;
		if(SM_PAmptitude != 0) delete [] SM_PAmptitude ;
	...
	for (std::multimap<uint32, Aura*>::iterator itr=m_aurascast.begin(); itr!=m_aurascast.end(); ++itr)
	{
		...
		if (itr->second->m_spellProto->UniqueTargetBased & 2) //1 target at a time
			sEventMgr.AddEvent(itr->second, &Aura::Remove, true, EVENT_AURA_REMOVE, 1, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT | EVENT_FLAG_DELETES_OBJECT);
	}
	m_aurascast.clear();
...
//	float targetreach = pVictim->GetFloatValue(UNIT_FIELD_COMBATREACH);
	...
	float attackreach = (targetradius * targetscale) + (selfradius * selfscale) + selfreach + 1.5f;
						...
						case 15269:
							{
								if( CastingSpell == NULL )
									continue;//this should not ocur unless we made a fuckup somewhere
								if( !(CastingSpell->schoolMask & SCHOOL_MASK_SHADOW) || !( CastingSpell->c_is_flags & SPELL_FLAG_IS_DAMAGING ) )
									continue;
							}break;
...
bool Unit::SetAurDuration(uint32 spellId,Unit* caster,uint32 duration)
{
	...
	sEventMgr.ModifyEventTimeLeft(aur, EVENT_AURA_REMOVE, duration + 500);
...
void Unit::CastSpell(GameObject* Target, SpellEntry* Sp, bool triggered)
{
	if( Sp == NULL )
		return;
...
void Unit::CastSpell(GameObject* Target, uint32 SpellID, bool triggered)
{
	SpellEntry * ent = dbcSpell.LookupEntry(SpellID);
	if(ent == 0) return;
...
bool Unit::IsDazed()
{
	for(uint32 x = 0; x < MAX_AURAS; ++x)
	{
		if(m_auras[x])
		{
			if(m_auras[x]->GetSpellProto()->mechanics == MECHANIC_ENSNARED)
			return true;
...
bool Unit::GetSpeedDecrease()
{
	...
	m_slowdown = 0;
	map< uint32, float >::iterator itr = speedReductionMap.begin();
...
void Unit::ModifyBonuses(uint32 type,int32 val)
{
	...
	switch (type) 
		...
		case POWER:
			...
			if (IsPlayer())
				static_cast<Player*>(this)->m_manafromitems += val;
			break;
		case HEALTH:
			ModUnsigned32Value( UNIT_FIELD_MAXHEALTH, val );
			if (IsPlayer())
				static_cast<Player*>(this)->m_healthfromitems += val;
			break;
