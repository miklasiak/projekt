...
typedef EventScript*(*exp_create_event)(Object* o, AreaTrigger* a, Spell * s);
...
typedef HM_NAMESPACE::hash_map<uint32, exp_create_event> EventCreateMap;
...
public:
	...
	SpellScript * CreateScriptClassForSpell(uint32 uEntryId, Spell* pSpell);
	EventScript * CreateScriptClassForEvent(uint32 uEntryId, Object* o, AreaTrigger* a, Spell* s);
	...
	void register_event_script(uint32 entry, exp_create_event callback);
...
class SERVER_DECL EventScript : public EventableObject
{
public:
	EventScript(Object* TriggeringObject = NULL, AreaTrigger* TriggeringAreaTrigger = NULL, Spell* TriggeringSpell = NULL) {}
	~EventScript();

	virtual void Update() {}
	void RegisterEventUpdate(uint32 time);
	void RemoveEventUpdate();
	void ModifyEventUpdate(uint32 newtime);
};
...
public:
	...
	virtual bool IsValidTarget(Object* obj) { return false; }
	virtual void OnAuraAdd(Aura* a) {}

	void RegisterSpellUpdate(uint32 time);
	void RemoveSpellUpdate();
	void ModifySpellUpdate(uint32 newtime);
	...
	virtual void DummyAura(bool apply, Aura* aura) {}
};
