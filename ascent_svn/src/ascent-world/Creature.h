...
public:
	...
	bool CanSee(Unit* obj) // * Invisibility & Stealth Detection - Partha *
	{
		...
		if(obj->m_auracount[SPELL_AURA_MOD_INVISIBILITY]) // Invisibility - Detection of Players and Units
		{
	...
	ASCENT_INLINE Unit* GetSummonOwner() { return summonOwner; }
	ASCENT_INLINE void SetSummonOwner(Unit *owner) { summonOwner = owner; }
	ASCENT_INLINE uint32 GetSummonSlot() { return summonSlot; }
	ASCENT_INLINE void SetSummonSlot(uint32 slot) { summonSlot = slot; }
	...
	ASCENT_INLINE bool IsTotem() { return summonOwner != NULL && (summonSlot > 0 && summonSlot < 5); }
	...
	void OnPushToWorld();
	void Despawn(int32 delay, uint32 respawntime);
