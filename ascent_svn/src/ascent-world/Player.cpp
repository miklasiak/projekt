...
void Player::EventTaxiDismount(TaxiPathNode* pn)
{
	...
	if (node == NULL || node->arrivalEventId == 0)
		return;
...
//	ss << LoadAuras;
	...
	ss << "','";
	std::set<uint32>::iterator fd = m_finisheddailyQuests.begin();
	for(; fd != m_finisheddailyQuests.end(); ++fd)
	{
		ss << (*fd) << ",";
	}
...
void Player::KillPlayer()
{
	...
	//remove guardians!
	for (std::set<Creature*>::iterator itr=m_guardians.begin(); itr!=m_guardians.end(); ++itr)
		(*itr)->SafeDelete();
	m_guardians.clear();
...
{   
	...
	if( cl != WARRIOR && cl != ROGUE && cl != DEATHKNIGHT )
	{
		...
		const float baseRegen[70] = { 0.034965f, 0.034191f, 0.033465f, 0.032526f, 0.031661f, 0.031076f, 0.030523f, 0.029994f, 0.029307f, 0.028661f, 
		0.027584f, 0.026215f, 0.025381f, 0.024300f, 0.023345f, 0.022748f, 0.021958f, 0.021386f, 0.020790f, 0.020121f, 0.019733f, 0.019155f, 0.018819f, 
		0.018316f, 0.017936f, 0.017576f, 0.017201f, 0.016919f, 0.016581f, 0.016233f, 0.015994f, 0.015707f, 0.015464f, 0.015204f, 0.014956f, 0.014744f, 
		0.014495f, 0.014302f, 0.014094f, 0.013895f, 0.013724f, 0.013522f, 0.013363f, 0.013175f, 0.012996f, 0.012853f, 0.012687f, 0.012539f, 0.012384f, 
		0.012233f, 0.012113f, 0.011973f, 0.011859f, 0.011714f, 0.011575f, 0.011473f, 0.011342f, 0.011245f, 0.011110f, 0.010999f, 0.010700f, 0.010522f, 
		0.010290f, 0.010119f, 0.009968f, 0.009808f, 0.009651f, 0.009553f, 0.009445f, 0.009327f };
	...
	if (pObj->IsUnit())
	{
		...
		if (GetSession() != NULL)
		{
			for (uint32 i=0; i<MAX_AURAS; ++i)
			{
				if (u->m_auras[i] != NULL)
				{
					uint8 flags = u->m_auras[i]->IsPositive()? 0x1F : 0x09;
					if (u->m_auras[i]->GetDuration() > 0)
						flags |= 0x20;
					WorldPacket data(SMSG_AURA_DATA, 28);
					FastGUIDPack(data, u->GetGUID());
					data << u->m_auras[i]->m_visualSlot; //flags unknown
					data << u->m_auras[i]->m_spellProto->Id;
					data << u->m_auras[i]->m_stackcount;
					data << flags; //flags2 unknown
					if (flags & 0x20)
					{
						data << u->m_auras[i]->GetDuration();
						data << u->m_auras[i]->GetTimeLeft();
					}
					GetSession()->SendPacket(&data);
				}
			}
...
int32 Player::CanShootRangedWeapon( uint32 spellid, Unit* target, bool autoshot )
{
	...
	float boundingradius = GetFloatValue(UNIT_FIELD_BOUNDINGRADIUS) + target->GetFloatValue(UNIT_FIELD_BOUNDINGRADIUS);
...
void Player::TaxiStart(TaxiPath *path, uint32 modelid, uint32 start_node)
{
	...
	if (node != NULL && node->departureEventId != 0)
		EventScript* e=sScriptMgr.CreateScriptClassForEvent(node->departureEventId, this, NULL, NULL);
...
*/
		...
		if (sp == NULL || sp->c_is_flags & SPELL_FLAG_IS_EXPIREING_WITH_PET)
		...
		if(sp->Id == 8326 || sp->Id == 9036 || sp->Id == 20584 || sp->Id == 15007)	// death auras
		{
			...
			a = new Aura(sp, GetDuration(sd), this, this);
			a->SetNegative();
		}
		else
		{
	...
	// useless logon spell
	SpellEntry* sp=dbcSpell.LookupEntry(836);
...
//	for(uint32 x=0;x<MAX_AURAS+MAX_PASSIVE_AURAS;x++)
	for(uint32 x=0;x<MAX_AURAS;x++)
	{
		if(m_auras[x])
		{
			...
			if (aur->GetDuration() == -1 && !(aur->m_spellProto->attributesExC & ATTRIBUTESEXC_CAN_PERSIST_AND_CASTED_WHILE_DEAD))
				skip = true;
