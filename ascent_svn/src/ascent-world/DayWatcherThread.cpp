...
void DayWatcherThread::update_settings()
{
	...
	CharacterDatabase.Execute("REPLACE INTO server_settings VALUES(\"last_dailyquest_update_time\", %u)", last_dailyquest_time);
}
...
void DayWatcherThread::load_settings()
{
	...
	string dailyquests_timeout = Config.MainConfig.GetStringDefault("Periods", "DailyQuestUpdate", "daily");
	dailyquest_period = get_timeout_from_string(dailyquests_timeout.c_str(), DAILY);
	...
	QueryResult * result2 = CharacterDatabase.Query("SELECT setting_value FROM server_settings WHERE setting_id = \"last_dailyquest_update_time\"");
	if(result2)
	{
		last_dailyquest_time = result2->Fetch()[0].GetUInt32();
		delete result2;
	}
	else
		last_dailyquest_time = 0;
}
...
bool DayWatcherThread::run()
{
	...
	while(ThreadState != THREADSTATE_TERMINATE)
	{
		...
		if(has_timeout_expired(&local_currenttime, &local_last_dailyquest_time, dailyquest_period))
			update_dailyquests();
		...
		do
		{
			...
			if(orig_arenapoints != arenapoints)
			{
				...
				if(plr)
				{
					...
					if (plr->m_arenaPoints > 5000)
						plr->m_arenaPoints = 5000;
...
void DayWatcherThread::update_dailyquests()
{
	CharacterDatabase.Execute("update `characters` set `finished_daily_quests` = ''");
	objmgr._playerslock.AcquireReadLock();	
	for (PlayerStorageMap::const_iterator itr=objmgr._players.begin(); itr!=objmgr._players.end(); ++itr)
	{
		sEventMgr.AddEvent(itr->second, &Player::WipeDailyQuests, EVENT_DAILY_UPDATE, 1000, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}
