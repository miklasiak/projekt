...
EventableObjectHolder::~EventableObjectHolder()
{
	...
	m_deletePoolLock.Acquire();
	TimedEventSet::iterator iqi;
	TimedEventSet::iterator iqi2 = m_deletePool.begin();
	while(iqi2 != m_deletePool.end())
	{
		iqi = iqi2++;
		delete (*iqi)->cb;
		delete (*iqi);
		m_deletePool.erase(iqi);
	}
	m_deletePoolLock.Release();
	m_lock.Release();
}
...
void EventableObjectHolder::Update(uint32 time_difference)
{
	...
	//new delete pool, for safe removal (events can remove themselves)
	m_deletePoolLock.Acquire();
	TimedEventSet::iterator iqi3;
	TimedEventSet::iterator iqi4 = m_deletePool.begin();
	while(iqi4 != m_deletePool.end())
	{
		iqi3 = iqi4++;
		delete (*iqi3)->cb;
		delete (*iqi3);
		m_deletePool.erase(iqi3);
	}
	m_deletePoolLock.Release();
	...
	while(itr != m_events.end())
	{
		...
		if((uint32)ev->currTime <= time_difference)
		{
			...
			{
				//fucking spell haste!
				while (ev->currTime <= time_difference && !ev->deleted)
				{
					ev->cb->execute();
