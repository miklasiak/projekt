...
AddItemResult ItemInterface::m_AddItem(Item *item, int8 ContainerSlot, int8 slot)
{
	...
	if ( slot < EQUIPMENT_SLOT_END && ContainerSlot == INVENTORY_SLOT_NOT_SET )
	{
		...
		else
		{
			...
			m_pOwner->SetUInt32Value( VisibleBase + 1, item->GetUInt32Value( ITEM_FIELD_ENCHANTMENT_1_1 ) ); // perm enchant
			m_pOwner->SetUInt32Value( VisibleBase + 2, item->GetUInt32Value( ITEM_FIELD_ENCHANTMENT_1_1 + 3 ) ); // temp enchant
			m_pOwner->SetUInt32Value( VisibleBase + 3, item->GetUInt32Value( ITEM_FIELD_ENCHANTMENT_1_1 + 6 ) ); // gem 1
			m_pOwner->SetUInt32Value( VisibleBase + 4, item->GetUInt32Value( ITEM_FIELD_ENCHANTMENT_1_1 + 9 ) ); // gem 2
			m_pOwner->SetUInt32Value( VisibleBase + 5, item->GetUInt32Value( ITEM_FIELD_ENCHANTMENT_1_1 + 12 ) ); // gem 3
			...
			if(type == INVTYPE_WEAPON || type == INVTYPE_WEAPONOFFHAND)
			{
				...
				if(mainweapon) //item exists
				{
					if(mainweapon->GetProto())
					{
						...
						{
							if(skip_2h_check) //titan's grip, need to find out 
								return 0;
							else
...
void ItemInterface::BuyItem(ItemPrototype *item, uint32 total_amount, Creature * pVendor)
{
	...
	CreatureItem citem;
	pVendor->GetSellItemByItemId(item->ItemId, citem);

	if( citem.extended_cost != NULL )
	{
		ItemExtendedCostEntry * ex = citem.extended_cost;
		for(int i = 0;i<5;i++)
		{
			if(ex->item[i])
				m_pOwner->GetItemInterface()->RemoveItemAmt( ex->item[i], total_amount * ex->count[i] );
...
bool ItemInterface::HasGemEquipped( uint32 GemID , int8 IgnoreSlot /* = -1 */ )
{
	Item * itm;
	EnchantEntry * ench;
	for( int8 x = EQUIPMENT_SLOT_START; x <= EQUIPMENT_SLOT_END; ++x )
	{
		if( x == IgnoreSlot )
			continue;
		if( m_pItems[x] != NULL )
