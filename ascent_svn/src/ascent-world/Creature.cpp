...
Creature::~Creature()
{
	...
	if(summonOwner != NULL && summonSlot != -1)
	{
		summonOwner->m_summonslot[summonSlot] = 0;
		summonOwner = NULL;
		summonSlot = -1;
	}
...
void Creature::SafeDelete()
{
	...
	if (IsPet())
		static_cast<Pet*>(this)->ClearPetOwner();

	sEventMgr.AddEvent(this, &Creature::DeleteMe, EVENT_CREATURE_SAFE_DELETE, 30000, 1, 0);
...
void Creature::RemoveFromWorld(bool addrespawnevent, bool free_guid)
{
	...
	if(IsInWorld())
	{
		...
		{
			Unit* u = GetMapMgr()->GetUnit(GetUInt64Value(UNIT_FIELD_SUMMONEDBY));
			if (u)
				u->m_guardians.erase(this);
		}
...
{
	if(summonOwner == pObj)		// player gone out of range of the totem
	{
		// Expire next loop.
...
void Creature::Despawn(int32 delay, uint32 respawntime)
{
	if(delay != -1)
	{
		sEventMgr.AddEvent(this, &Creature::Despawn, -1, respawntime, EVENT_CREATURE_RESPAWN, delay, 1,0);
		return;
	}
