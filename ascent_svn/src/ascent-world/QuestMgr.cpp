...
uint32 QuestMgr::PlayerMeetsReqs(Player* plr, Quest* qst, bool skiplevelcheck)
{
	...
	if (plr->HasFinishedQuest(qst->id) && qst->is_repeatable == 0)
		return QMGR_QUEST_NOT_AVAILABLE;
...
bool QuestMgr::OnActivateQuestGiver(Object *qst_giver, Player *plr)
{
	...
	else if (questCount == 1)
	{
		...
		if (status == QMGR_QUEST_AVAILABLE || status == QMGR_QUEST_REPEATABLE || status == QMGR_QUEST_CHAT || status == QMGR_QUEST_REPEATABLE_FINISHED)
		{
