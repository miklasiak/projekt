...
//	void HandleProcSpellOnSpell(Unit* Victim,uint32 damage,bool critical);//nasty, some spells proc other spells
	...
	void CastSpell(GameObject* Target, uint32 SpellID, bool triggered);
	void CastSpell(GameObject* Target, SpellEntry* Sp, bool triggered);
...
//	std::map<uint32,ProcTriggerSpellOnSpellList> m_procSpellonSpell; //index is namehash
	...
	ASCENT_INLINE int GetHealthPct() { if (GetUInt32Value(UNIT_FIELD_MAXHEALTH)==0)  return 0; return (int)(GetUInt32Value(UNIT_FIELD_HEALTH) * 100 / GetUInt32Value(UNIT_FIELD_MAXHEALTH)); };
	...
	ASCENT_INLINE int GetManaPct() { if (GetUInt32Value(UNIT_FIELD_MAXPOWER1)==0)  return 0; return (int)(GetUInt32Value(UNIT_FIELD_POWER1) * 100 / GetUInt32Value(UNIT_FIELD_MAXPOWER1)); };
