...
void WorldSession::HandleAcceptTrade(WorldPacket & recv_data)
{
	...
	if(plr->mTradeStatus == TRADE_STATUS_ACCEPTED)
	{
		...
		for(uint32 Index = 0; Index < 7; ++Index)
		{
			...
			{
				Item * pItem = _player->mTradeItems[Index];
			...
			{
				Item * tItem = pTarget->mTradeItems[Index];
				if( tItem->IsContainer() && ((Container*)tItem)->HasItems() )
				{
					sCheatLog.writefromsession(this, "%s involved in bag-trick trade.", pTarget->GetName());
					pTarget->GetItemInterface()->BuildInventoryChangeError(	tItem, NULL, INV_ERR_CANT_TRADE_EQUIP_BAGS);
					TradeStatus = TRADE_STATUS_CANCELLED;
				}
				++TargetItemCount;
			}
		...
		else if ( TradeStatus != TRADE_STATUS_CANCELLED )
		{
			uint64 Guid;
			Item * pItem;
