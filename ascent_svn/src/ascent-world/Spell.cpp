...
Spell::Spell(Object* Caster, SpellEntry *info, bool triggered, Aura* aur)
{
	...
	switch( Caster->GetTypeId() )
	{
		...
		case TYPEID_UNIT:
        {
		    ...
		    p_caster = NULL;
		    u_caster = static_cast< Unit* >( Caster );
			if( u_caster->IsPet() && static_cast< Pet* >(u_caster)->GetPetOwner() != NULL && static_cast< Pet* >(u_caster)->GetPetOwner()->IsPlayer() && static_cast<Player*>(static_cast< Pet* >( u_caster )->GetPetOwner())->GetDuelState() == DUEL_STATE_STARTED )
			    duelSpell = true;
        }break;
...
uint8 Spell::DidHit(uint32 effindex,Unit* target)
{
	...
	/************************************************************************/
	if(u_victim->SchoolImmunityList[spellschool])
	...
	if( m_spellInfo->is_melee_spell || m_spellInfo->is_ranged_spell )
	{
		...
		melee_test_result = u_caster->GetSpellDidHitResult( u_victim, _type, m_spellInfo, spellschool );
...
uint8 Spell::prepare( SpellCastTargets * targets )
{
	...
	if (m_targets.m_targetMask & (TARGET_FLAG_OBJECT | TARGET_FLAG_UNIT | TARGET_FLAG_CORPSE | TARGET_FLAG_CORPSE2 | TARGET_FLAG_ITEM | TARGET_FLAG_TRADE_ITEM) && m_targets.m_target == NULL)
	{
		finish();
...
void Spell::cast(bool check)
{
	if( duelSpell && (
		...
		( u_caster != NULL && u_caster->IsPet() && static_cast< Pet* >( u_caster )->GetPetOwner() && static_cast< Pet* >( u_caster )->GetPetOwner()->IsPlayer() && static_cast<Player*>(static_cast< Pet* >( u_caster )->GetPetOwner())->GetDuelState() != DUEL_STATE_STARTED ) ) )
	{
		// Can't cast that!
	...
	if(cancastresult == SPELL_CANCAST_OK)
	{
		if (p_caster && !m_triggeredSpell && p_caster->IsInWorld() && m_targets.m_target != NULL && GET_TYPE_FROM_GUID(m_targets.m_target->GetGUID())==HIGHGUID_TYPE_UNIT)
		{
			sQuestMgr.OnPlayerCast(p_caster, m_spellInfo->Id, m_targets.m_target->GetGUID());
		}
			...
			{			
				if( m_spellInfo->channelInterruptFlags != 0 && !m_triggeredSpell)
				{
					...
					*/
					...
					if (u_caster != NULL)
						channelDuration *= u_caster->GetFloatValue(UNIT_MOD_CAST_SPEED);
...
//		float(m_castTime)/float(m_timer) >= 2.0f		&&
	...
	if (m_targets.m_target != NULL && m_targets.m_target->IsUnit() && !static_cast<Unit*>(m_targets.m_target)->isAlive())
	{
		cancel();
		return;
	}
...
void Spell::SendCastResult(uint8 result)
{
	...
	switch( result )
	{
		...
		case SPELL_FAILED_REQUIRES_AREA:
			Extra = m_spellInfo->requiredAreaID;
			break;
...
void Spell::SendSpellGo()
{
	...
	uint8 ObjectsHit = 0;
	uint8 ObjectsModerated = 0;

	for (SpellTargetMap::iterator itr=m_spellTargets.begin(); itr!=m_spellTargets.end(); ++itr)
	{
		if (itr->second.TargetModType == 0)
			++ObjectsHit;
		else
			++ObjectsModerated;
	}
	...
	data << flags;
	data << getMSTime();
	data << (uint8)(ObjectsHit); //number of hits
	
	writeSpellGoTargets( &data );
	data << (uint8)ObjectsModerated; //number if misses
	if(flags & 0x400)
...
void Spell::SendInterrupted(uint8 result)
{
	...
	Player *plr = p_caster;
	if(!plr && m_caster->IsPet() && static_cast<Pet*>(m_caster)->GetPetOwner()->IsPlayer())
		plr = static_cast<Player*>(static_cast<Pet*>(m_caster)->GetPetOwner());
	if(!plr && u_caster)
		plr = u_caster->m_redirectSpellPackets;
...
void Spell::HandleAddAura(Unit* u, Aura* a)
{
	...
	if(u->IsPlayer() && p_caster != NULL && p_caster != u)
	{
	...
	if( m_spellInfo->mechanics == 25 && m_spellInfo->Id != 25771) // Cast spell Forbearance
		spellid = 25771;
...
uint8 Spell::CanCast(bool tolerate)
{
	...
	if( p_caster != NULL )
	{
...
#ifdef COLLISION
		if (m_spellInfo->mechanics == MECHANIC_MOUNTED)
		{
		...
		if (m_spellInfo->requiredAreaID && m_spellInfo->requiredAreaID != p_caster->GetMapMgr()->GetAreaID(p_caster->GetPositionX(),p_caster->GetPositionY()))
	...
	if(m_targets.m_target && m_targets.m_target->IsItem() && p_caster)
	{
		Item *i_target = NULL;
		...
		switch(m_spellInfo->Effect[0])
		{
			...
			case SPELL_EFFECT_ENCHANT_ITEM_TEMPORARY:
			{
				...
				if( m_spellInfo->attributesExB & ATTRIBUTESEXB_ENCHANT_OWN_ONLY && !(i_target->IsSoulbound()))
					return SPELL_FAILED_BAD_TARGETS;
	...
	float maxRange = GetMaxRange( dbcSpellRange.LookupEntry( m_spellInfo->rangeIndex ) );
		...
		if( u_caster && m_targets.m_target && m_targets.m_target->IsUnit() )
		{
	...
	if(m_targets.m_target != NULL && m_targets.m_target->IsUnit())
	{
		...
		if(target)
		{
			float totalboundradius = target->GetFloatValue(UNIT_FIELD_BOUNDINGRADIUS);
			if (u_caster != NULL)
				totalboundradius += u_caster->GetFloatValue(UNIT_FIELD_BOUNDINGRADIUS);

			if( tolerate ) // add an extra 33% to range on final check (squared = 1.78x)
			{
				if( !IsInrange( m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), target, ( ( maxRange + totalboundradius ) * ( maxRange + totalboundradius ) * 1.78f ) ) )
					return SPELL_FAILED_OUT_OF_RANGE;
			}
			else
			{
				if( !IsInrange( m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ(), target, ( ( maxRange + totalboundradius ) * ( maxRange + totalboundradius ) ) ) )
					return SPELL_FAILED_OUT_OF_RANGE;
			}
			...
			if(target != u_caster && !m_caster->IsPet())
			{
				...
				**********************************************************/
				...
				if(p_caster)
				{
					...
					{
						if(m_spellInfo->in_front_status == 2 && target->isInFront(u_caster))
							return SPELL_FAILED_NOT_BEHIND;
						else if (!p_caster->isInFront(target))
			...
			if( p_caster != NULL )
			{
				...
				//if(target->MechanicsDispels[m_spellInfo->mechanics])
	...
	if (m_spellInfo->mechanics == 25 && ((target) ? target->HasAura(25771) : u_caster->HasAura(25771))) //Forbearance
	...
	if( m_spellInfo->NameHash == SPELL_HASH_AVENGING_WRATH && ((target) ? target->HasAura(25771) : u_caster->HasAura(25771))) // forbearance, avenging wrath
		return SPELL_FAILED_DAMAGE_IMMUNE;
	if (m_spellInfo->mechanics == 16 && ((target) ? target->HasAura(11196) : u_caster->HasAura(11196)))
		return SPELL_FAILED_DAMAGE_IMMUNE;
...
void Spell::RemoveItems()
{
	...
	// Ammo Removal
	if( m_spellInfo->attributesExB == ATTRIBUTESEXB_REQ_RANGED_WEAPON || m_spellInfo->attributesExC & ATTRIBUTESEXC_PLAYER_RANGED_SPELLS)
	{
...
void Spell::EventDelete()
{
	...
	if (!sEventMgr.HasEvent(this, EVENT_SPELL_HIT) && m_spellScript == NULL)
		delete this;
}
