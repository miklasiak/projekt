...
class AkamaAI : public MoonScriptBossAI
{
	...
	void IllidanDialogEvent()
	{
		if (mIllidanAI == NULL)
		{
			Creature *pIllidan = (Creature*)(ForceCreatureFind(22917, 704.539001f, 305.282013f, 353.919006f));
	...
	Unit *FindClosestTargetToUnit(Unit *pSeeker)
	{
		...
		for (std::set<Player*>::iterator itr = _unit->GetInRangePlayerSetBegin(); itr != _unit->GetInRangePlayerSetEnd(); ++itr)
		{
			...
			//if(_unit->m_invisible = true)					// Arc's)
				continue;
...
class FlameOfAzzinothAI : public MoonScriptCreatureAI
{
	...
	void Destroy()
	{
		Creature *pIllidan = (Creature*)(ForceCreatureFind(22917));
		if (pIllidan != NULL && pIllidan->GetScript() != NULL)
		{
