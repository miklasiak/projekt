...
void AccountMgr::AddAccount(Field* field)
{
	...
	if( EncryptedPassword.size() > 0 )
	{
		...
		{
			printf("Account `%s` has incorrect number of bytes (%u) in encrypted password string! Disabling.\n", Username.c_str(), bn.GetNumBytes());
		...
		else
		{
			...
			//0 is optimized out from bignumber, memcpy 0 to the rest of the space!
			for (uint8 i=bn.GetNumBytes(); i<20; ++i)
				acct->SrpHash[i] = 0;
...
void InformationCore::TimeoutSockets()
{
	...
	for(itr = m_serverSockets.begin(); itr != m_serverSockets.end();)
	{
		...
		if(s->last_ping < t && ((t - s->last_ping) > 240))
		{
			...
			s->removed = true;
			set<uint32>::iterator itr2 = s->server_ids.begin();
			for(; itr2 != s->server_ids.end(); ++itr2)
				RemoveRealm(*itr2);
